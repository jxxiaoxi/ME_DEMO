...
public class VTManagerLocal {
    ...
    public static final String LOG_TAG = "VTManagerLocal";

    // messages which will be used in UI, sync with VTManager's defines.
    ...
    // messages which will be used in InCallUI and Telephony, sync with VTManagerWrapper's defines.
    ...
    // messages used to notify UI through VTListener when pushVTManagerParams() and pushVTSettingParams() arrived.
    // only defined and used in InCallUI.
    public static final int VT_MANAGER_PARAMS_UPDATE = 130;
    public static final int VT_SETTING_PARAMS_UPDATE = 131;

    // Back for VTManager's state
    public enum State {
        CLOSE, OPEN, READY, CONNECTED
    }

    private static VTManagerLocal sVTManagerLocal = new VTManagerLocal();
    private State mState = State.CLOSE;

    // below are camera parameters from VTManager, sync with VTManager and will not be changed in InCallUI.
    ...
    public static VTManagerLocal getInstance() {
        return sVTManagerLocal;
    }

    /**
     * handle cared messages from VTManager.
     * @param msgVT
     */
    public void onVTStateChanged(int msgVT) {
        ...
        case VT_MSG_DISCONNECTED:
            VTUIFlags.getInstance().mVTHasReceiveFirstFrame = false;
            break;

        case VT_MSG_START_COUNTER:
            /// For VT time count. see ALPS01425992.
            if (VTUIFlags.getInstance().mVTConnectionStarttime.mStarttime < 0) {
                VTUIFlags.getInstance().mVTConnectionStarttime.mStarttime = SystemClock
                        .elapsedRealtime();
            }
            break;

        case VT_MSG_RECEIVE_FIRSTFRAME:
            VTUIFlags.getInstance().mVTHasReceiveFirstFrame = true;
            break;
        ...
        case VT_RESULT_SWITCHCAMERA_FAIL:
            VTUIFlags.getInstance().mVTInSwitchCamera = false;
            Log.i(this, "onVTSateChanged() / mVTInSwitchCamera : " + VTUIFlags.getInstance().mVTInSwitchCamera);
            break;

        case VT_RESULT_PEER_SNAPSHOT_OK:
        case VT_RESULT_PEER_SNAPSHOT_FAIL:
            VTUIFlags.getInstance().mVTInSnapshot = false;
            break;

        default:
            break;
        }
        // pass those messages to other parts which are interesting with them.
    ...
    public void getVTManagerParams(VTManagerParams params) {
        ...
        // notify other parts that we have receive new VTManager parameters.
        notifyVTStateChange(VT_MANAGER_PARAMS_UPDATE);
    }
    ...
    private void logVTMsg(int msgVT) {
        String msgVTString = "unkown message";
        switch (msgVT) {
        case VT_MSG_CLOSE:
            msgVTString = "VT_MSG_CLOSE";
            break;
        case VT_MSG_OPEN:
            msgVTString = "VT_MSG_OPEN";
            break;
        case VT_MSG_READY:
            msgVTString = "VT_MSG_READY";
            break;
        case VT_MSG_CONNECTED:
            msgVTString = "VT_MSG_CONNECTED";
            break;
        case VT_MSG_DISCONNECTED:
            msgVTString = "VT_MSG_DISCONNECTED";
            break;
        case VT_MSG_START_COUNTER:
            msgVTString = "VT_MSG_START_COUNTER";
            break;
        case VT_MSG_RECEIVE_FIRSTFRAME:
            msgVTString = "VT_MSG_RECEIVE_FIRSTFRAME";
            break;
        case VT_MSG_EM_INDICATION:
            msgVTString = "VT_MSG_EM_INDICATION";
            break;
        case VT_MSG_PEER_CAMERA_OPEN:
            msgVTString = "VT_MSG_PEER_CAMERA_OPEN";
            break;
        case VT_MSG_PEER_CAMERA_CLOSE:
            msgVTString = "VT_MSG_PEER_CAMERA_CLOSE";
            break;
        case VT_MSG_CAM_BEGIN:
            msgVTString = "VT_MSG_CAM_BEGIN";
            break;
        case VT_ERROR_CALL_DISCONNECT:
            msgVTString = "VT_ERROR_CALL_DISCONNECT";
            break;
        case VT_ERROR_START_VTS_FAIL:
            msgVTString = "VT_ERROR_START_VTS_FAIL";
            break;
        case VT_ERROR_CAMERA:
            msgVTString = "VT_ERROR_CAMERA";
            break;
        case VT_ERROR_MEDIA_SERVER_DIED:
            msgVTString = "VT_ERROR_MEDIA_SERVER_DIED";
            break;
        case VT_ERROR_MEDIA_RECORDER_EVENT_INFO:
            msgVTString = "VT_ERROR_MEDIA_RECORDER_EVENT_INFO";
            break;
        case VT_ERROR_MEDIA_RECORDER_EVENT_ERROR:
            msgVTString = "VT_ERROR_MEDIA_RECORDER_EVENT_ERROR";
            break;
        case VT_ERROR_MEDIA_RECORDER_COMPLETE:
            msgVTString = "VT_ERROR_MEDIA_RECORDER_COMPLETE";
            break;
        case VT_NORMAL_END_SESSION_COMMAND:
            msgVTString = "VT_NORMAL_END_SESSION_COMMAND";
            break;
        case VT_RESULT_SWITCHCAMERA_OK:
            msgVTString = "VT_RESULT_SWITCHCAMERA_OK";
            break;
        case VT_RESULT_SWITCHCAMERA_FAIL:
            msgVTString = "VT_RESULT_SWITCHCAMERA_FAIL";
            break;
        case VT_RESULT_PEER_SNAPSHOT_OK:
            msgVTString = "VT_RESULT_PEER_SNAPSHOT_OK";
            break;
        case VT_RESULT_PEER_SNAPSHOT_FAIL:
            msgVTString = "VT_RESULT_PEER_SNAPSHOT_FAIL";
            break;
        default:
            break;
        }
        Log.d(this, "onVTStateChanged()... msgVT: " + msgVT + " (" + msgVTString + ").");
    }
