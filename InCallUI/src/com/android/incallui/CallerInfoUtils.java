...
public class CallerInfoUtils {
    ...
    public static CallerInfo getCallerInfoForCall(Context context, CallIdentification call,
        ...
        /// for VoLTE PAU field @{
        // if no number in Call, use number in PAU instead.
        if (FeatureOptionWrapper.isSupportVoLte() && TextUtils.isEmpty(number)) {
            String numberInPAU = VoLteUtils.getNumberFromPAU(call.getPAUField());
            String sipNumberInPAU = VoLteUtils.getSipNumberFromPAU(call.getPAUField());
        ...
        /// M: For ALPS01537419 CNAP support, the number is null when support CNAP.
        // should not find info by number if the number is invaild. @{
        boolean numberInvaild = TextUtils.isEmpty(number);
        if (!numberInvaild && info.numberPresentation == Call.PRESENTATION_ALLOWED) {
            ///@}
