...
import com.mediatek.incallui.volte.VoLteUtils;
...
public class CallCardPresenter extends Presenter<CallCardPresenter.CallCardUi>
    ...
    public void onStateChange(InCallState state, CallList callList) {
        ...
        VTScreenMode vtScreenMode = VTUtils.getVTScreenMode();
        switch (vtScreenMode) {
            case VT_SCREEN_OPEN:
        ...
        */
        ...
        if (mPrimary != null && mPrimary.getState() == Call.State.ACTIVE) {
            ...
            } else if (VTUIFlags.getInstance().mVTConnectionStarttime.mStarttime >= 0) {
                mCallTimer.start(CALL_TIME_UPDATE_INTERVAL);
            } else {
        ...
        if (mPrimary != null) {
            ...
            /// @}
            /// M: Plug-in 
            ExtensionManager.getCallCardExt().onStateChange(mPrimary);
            ExtensionManager.getRCSeCallCardExt().onStateChange(mPrimary);
        } else {
    ...
    */
    ...
    private void updatePrimaryDisplayInfo(ContactCacheEntry entry, boolean isConference) {
        ...
        /// M: Add for plugin. @{
        ExtensionManager.getCallCardExt().updatePrimaryDisplayInfo(mPrimary);
    }
    ...
    private static String getNameForCall(ContactCacheEntry contactInfo) {
        if (TextUtils.isEmpty(contactInfo.name)) {
            ...
            return getNumberForVolTE(contactInfo);
            /// @}
        }
...
//            return contactInfo.location;
        ...
        /// M: for VoLTE PAU field @{
        // VoLTE TODO: follow UI spec to parse and replace the pau field
        // Original Code:
        // return contactInfo.number;
        return getNumberForVolTE(contactInfo);
        /// @}
    ...
    /// M: for VoLTE PAU field @{
    private static String getNumberForVolTE(ContactCacheEntry contactInfo) {
        String number = contactInfo.number;
        if(FeatureOptionWrapper.isSupportVoLte() && !TextUtils.isEmpty(contactInfo.pau)) {
            String nameInCache = contactInfo.name;
            String nameInPAU = VoLteUtils.getNameFromPAU(contactInfo.pau);
            if (TextUtils.isEmpty(nameInCache)) {
                // name is empty, use number + name in pau if has.
                if (!TextUtils.isEmpty(nameInPAU)) {
                    number = number + " < " + nameInPAU + " >";
                }
            } else {
                // name is not null, if name in Cache is not same as name in PAU, use number + name in PAU.
                if (!TextUtils.isEmpty(nameInPAU) && !nameInPAU.equals(nameInCache)) {
                    number = number + " < " + nameInPAU + " >";
                }
            }
        }
    ...
    // check whether is waiting (in dialing state)
    public boolean isCallWaiting() {
        if (mPrimary != null) {
            return State.isDialing(mPrimary.getState()) && mPrimary.isWaitingCall();
        }
        return false;
    }
    /// @}
    ...
    public boolean isUpdatingVoLteConferenceCall() {
        boolean isVoLteConf = false;
        Call ringCall = null;
        ringCall = CallList.getInstance().getIncomingCall();
        if (ringCall != null) {
            isVoLteConf = ringCall.isVoLteConferenceCall();
        }
        return isVoLteConf;
    }
    /// @}
            ...
            if (VTTimingMode.VT_TIMING_NONE == VTUtils.checkVTTimingMode(call.getNumber())) {
                ...
                if (VTUIFlags.getInstance().mVTConnectionStarttime.mStarttime < 0) {
                    ...
                    return SystemClock.elapsedRealtime()
                            - VTUIFlags.getInstance().mVTConnectionStarttime.mStarttime;
                }
            } else {
