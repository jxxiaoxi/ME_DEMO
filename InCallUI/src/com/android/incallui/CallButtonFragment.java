...
public class CallButtonFragment
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		// add by liuwei for 5s srceen off
		View parent = null;
		isPhoneLock = SystemProperties.getBoolean("ro.phone.lock", false);
		if (SystemProperties.getBoolean("ro.phone.lockscreen_for_yl", false)) {
			mSlectLockForYL = Settings.System.getInt(getActivity()
					.getContentResolver(), "other_lock_key", 1);
			if (mSlectLockForYL == 1) {
				parent = inflater.inflate(R.layout.call_button_fragment_lock,
						container, false);
			} else {
				isPhoneLock = false;
				parent = inflater.inflate(R.layout.call_button_fragment,
						container, false);
			}

		} else {
			parent = inflater.inflate(R.layout.call_button_fragment, container,
					false);
		}
		Log.e("huiye","CallButtonFragment mSlectLockForYL : "+mSlectLockForYL);
        ...
        mManageConferenceButton.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
				// add by liuwei for 5s srceen off
				if (isPhoneLock) {
					mTouchScreenListener.onTouchScreen();
				}
        ...
        */
		...
		// add by liuwei for 5s srceen off
		if (mSlectLockForYL == 1
				&& SystemProperties.getBoolean("ro.phone.lockscreen_for_yl", false)) {
			mcallButtonLocked = parent.findViewById(R.id.call_button_locked);
			mCallLock = (ImageView) parent.findViewById(R.id.calllock);
			mCallControlsBg = (ImageView) parent
					.findViewById(R.id.inCallControlsBg);
			mCallLockText = (TextView) parent.findViewById(R.id.calllocktext);
			mCallLock.setOnTouchListener(this);
		}
    ...
    public void setAudio(int mode) {
        updateAudioButtons(getPresenter().getSupportedAudio());
        /// M: For ALPS01505606 @{
        // TeleService will trigger AudioMode popup refresh when supported Audio has been changed. Here we only update Audio Button.
        // Original Code:
    ...
    public void displayDialpad(boolean value) {
        ...
        if (getActivity() != null && getActivity() instanceof InCallActivity) {
            /// M: fix ALPS01852804, to show display manage-conf button.
            mIsDialpadShow = value;
            Log.d(this, "displayDialpad mIsDialpadShow = " + value);
    ...
    public boolean isDialpadVisible() {
        if (getActivity() != null && getActivity() instanceof InCallActivity) {
            /// M: fix ALPS01852804, to show display manage-conf button.
            /// Google code: return ((InCallActivity) getActivity()).isDialpadVisible();
    ...
    public void showManageConferenceCallButton() {
        ...
        mVoLteAddConferenceMemberButton.setVisibility(View.GONE);
        mVoLteManageConferenceButton.setVisibility(View.GONE);
        /// @}
    }
    ...
    private void onCreateViewMtk(View parent) {
        ...
        setupExtraButtonsForVoLte(parent);
    }
    ...
    private void initVTCallButton(View parent) {
        ...
        if (InCallUtils.hasPermanentMenuKey(getActivity()) && 
                !InCallUtils.isSmartBookPluggedIn(getActivity())) {
    ...
    private void setEnableForVT(boolean isEnabled) {
        ...
        if (isEnabled) {
            mVTSwapVideo.setEnabled(VTUIFlags.getInstance().mVTHasReceiveFirstFrame);
        }
        mVTSwapVideo.setChecked(!VTUIFlags.getInstance().mVTPeerBigger);
    }
    ...
    public void updateCallButton(boolean isVT) {
        ...
        if (isVT) {
			...
			// add by liuwei for 5s srceen off
			if (SystemProperties
					.getBoolean("ro.phone.lockscreen_for_yl", false)) {
				if (mSlectLockForYL == 1 && !isLocked()) {
...
//                final ToggleButton button = (ToggleButton) view;
            ...
            case R.id.VTOverflowMenu:
                ...
                break;
                /// M: for VoLTE Conference Call @{
            case R.id.voLteManageConferenceButton:
                getPresenter().manageVoLteConf();
                break;
            case R.id.voLteAddConferenceMemberButton:
                getPresenter().addVoLteConfMember();
                break;
            /// @}
            default:
                return false;
    ...
    public void setupVTMenuItems(Menu menu) {
        ...
        swapVideosMenu.setEnabled(VTUIFlags.getInstance().mVTHasReceiveFirstFrame);

        takePeerPhotoMenu.setEnabled(VTUIFlags.getInstance().mVTHasReceiveFirstFrame
    ...
    public void setupMenuItems(Menu menu) {
        ...
        if (getPresenter().isVTCall()) {
            ...
            setupVoiceMenuItems(menu);
            mRCSeExt.setupMenuItems(menu, getPresenter().getCall());
        }
    }
    ...
    /// M: for VoLTE conference call @{
    private ViewGroup mVoLteManageConferenceButton;
    private ViewGroup mVoLteAddConferenceMemberButton;

    private void setupExtraButtonsForVoLte(View parent) {
        // manage conference
        mVoLteManageConferenceButton = (ViewGroup) parent
                .findViewById(R.id.voLteManageConferenceButton);
        mVoLteManageConferenceButton.setOnClickListener(this);

        // add conference member
        mVoLteAddConferenceMemberButton = (ViewGroup) parent
                .findViewById(R.id.voLteAddConferenceMemberButton);
        mVoLteAddConferenceMemberButton.setOnClickListener(this);
    }

    @Override
    public void showVoLteAddConfMemberButton() {
        mExtraRowButton.setVisibility(View.VISIBLE);
        mVoLteAddConferenceMemberButton.setVisibility(View.VISIBLE);
    }

    @Override
    public void showVoLteManageConfCallButton() {
        mExtraRowButton.setVisibility(View.VISIBLE);
        mVoLteManageConferenceButton.setVisibility(View.VISIBLE);
        mManageConferenceButton.setVisibility(View.GONE);
        mGenericMergeButton.setVisibility(View.GONE);
    }

    @Override
    public void displayVoLteManageConfPanel(boolean value) {
        if (getActivity() != null && getActivity() instanceof InCallActivity) {
            ((InCallActivity) getActivity()).displayVoLteManageConfPanel(value);
        }
    }
    /// @}
	// add by liuwei for 5s srceen off
	private ImageView mCallLock;
	private ImageView mCallControlsBg;
	private TextView mCallLockText;
	private View mcallButtonLocked;
	private LinearLayout mCallBttonLock;
	private static int minslidx = 62;
	private static int slideunlock = 140;
	private int moffset = 0;
	private AbsoluteLayout.LayoutParams newLayoutParams;
	private int old_x;
	private int new_x;
	private boolean isFreeScreen;
	...
	public void picMove(int offset) {
		int intWidth = mCallLock.getWidth();
		int intHeight = mCallLock.getHeight();
		newLayoutParams = new AbsoluteLayout.LayoutParams(intWidth, intHeight,
				minslidx + offset, 120);
		mCallLock.setLayoutParams(newLayoutParams);
		if (offset > slideunlock)
			picUnclock();
	}
	...
	public void SetcallLock(boolean islock) {
		if (islock) {
			if (isDialpadVisible()) {
			...
			mEndCallButton.setVisibility(View.GONE);
			mVoiceButtonContainer.setVisibility(View.GONE);
			mcallButtonLocked.setVisibility(View.VISIBLE);
			mCallLock.setVisibility(View.VISIBLE);
			mCallControlsBg.setVisibility(View.VISIBLE);
			mCallLockText.setVisibility(View.VISIBLE);
			// 整个上面部分
		} else {
			mVoiceButtonContainer.setVisibility(View.VISIBLE);
			mEndCallButton.setVisibility(View.VISIBLE);
			mIsLocked = false;
			mcallButtonLocked.setVisibility(View.GONE);
			mCallLock.setVisibility(View.GONE);
			mCallControlsBg.setVisibility(View.GONE);
			mCallLockText.setVisibility(View.GONE);
			// add by liuwei for 5s srceen off
			if (mSlectLockForYL == 1) {
				mTouchScreenListener.isLockOrScreenOff(false);
			}
		}
	}
	...
	public void onDestroy() {
		// TODO Auto-generated method stub
		super.onDestroy();
		if (isPhoneLock && mIntentReceiver != null) {
			if(mSlectLockForYL==1){
				SetcallLock(false);
			}
	...
	public void setScreenSleep() {
		PowerManager pm = (PowerManager) getUi().getContext().getSystemService(
	...
	private final BroadcastReceiver mIntentReceiver = new BroadcastReceiver() {
		...
		public void onReceive(Context context, Intent intent) {
			if (intent.getAction().equals(Intent.ACTION_SCREEN_OFF)) {
				Log.e("huiye", "CallButtonFragment ACTION_SCREEN_OFF");
				if (mSlectLockForYL == 1 && !mIsLocked
						&& getPresenter().mIsCalling)
					SetcallLock(true);
			} else if (intent.getAction().equals(Intent.ACTION_SCREEN_ON)) {
				Log.e("huiye", "CallButtonFragment ACTION_SCREEN_ON  : "
						+ getPresenter().mIsCalling
						+ "  ; isPhoneInBackground : " + isPhoneInBackground);
				if (getPresenter().mIsCalling && (!isPhoneInBackground)) {
					mTouchScreenListener.isLockOrScreenOff(false);
				}
				if (mSlectLockForYL == 1 && !mIsLocked
						&& getPresenter().mIsCalling) {
	...
	public interface OnCallButtomTouchScreenListener {
		public void onTouchScreen();

		public void onCalling(boolean enabled);

		public void isLockOrScreenOff(boolean enabled);
	}

	OnCallButtomTouchScreenListener mTouchScreenListener;
	public boolean isPhoneInBackground;

	@Override
	public void onAttach(Activity activity) {
		// TODO Auto-generated method stub
		super.onAttach(activity);
		mTouchScreenListener = (OnCallButtomTouchScreenListener) activity;
	}

	@Override
	public void isCalling(boolean enabled) {
		// TODO Auto-generated method stub
		mTouchScreenListener.onCalling(enabled);
	}
}
