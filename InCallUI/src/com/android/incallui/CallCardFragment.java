...
//0011862拨号界面输入错误的超长号码，可拨出无任何提示，拨出界面直接出现通话结束 start
    ...
    private void setCallName(String name){
        callName = name;
    }
//0011862拨号界面输入错误的超长号码，可拨出无任何提示，拨出界面直接出现通话结束 end
    @Override
    public void setPrimary(String number, String name, boolean nameIsNumber, String label,
        ...
        //0011862拨号界面输入错误的超长号码，可拨出无任何提示，拨出界面直接出现通话结束
        setCallName(name);
    ...
    public void setCallState(int state, Call.DisconnectCause cause, boolean bluetoothOn,
        ...
        if (FeatureOptionWrapper.isSupportVoLte() && getPresenter().isCallWaiting()) {
            updateCallStateforCallWaiting();
        }
        /// @}
    ...
    private String getCallStateLabelFromState(int state, Call.DisconnectCause cause) {
        ...
        if (Call.State.IDLE == state) {
            ...
            } else if(FeatureOptionWrapper.isSupportVoLte() && getPresenter().isUpdatingVoLteConferenceCall()){
    ...
    private int[] mSimIndicatorIconMap = {
            R.drawable.sim_indicator_master,
            R.drawable.sim_indicator_slave,
        };
    ...
    public void setSimIndicator(int slotId, int callType) {
        ...
        if (simInfo != null && !TextUtils.isEmpty(simInfo.mDisplayName)
            ...
            //coolpad需求:通话界面显示使用的sim卡图标
            if (SystemProperties.getBoolean("ro.dual.sim.icon", true)) {
            	int simNum = SIMInfoWrapper.getDefault().getInsertedSimCount();
            	if (simNum > 1 && simInfo.mSimSlotId < mSimIndicatorIconMap.length) {
                	mSimIndicatorIcon.setVisibility(View.VISIBLE);
                    mSimIndicatorIcon.setImageResource(mSimIndicatorIconMap[simInfo.mSimSlotId]);
                } else {
    ...
    private void initVoiceRecorderIcon(View view) {
        ...
        /* add by zhanglongtao begin */
        mVoiceRecorderTimer = (TextView) view.findViewById(R.id.voiceRecorderTimer);
        if (mVoiceRecorderTimer != null) {
        	mVoiceRecorderTimer.setText("");
    	...
    	if(mTask == null) {
			...
			mTask = new TimerTask() {
        		...
        		public void run() {
        			...
        			message.what = PHONE_VOICE_RECORDING;
    ...
    public void onViewCreatedMTK(View view) {
        ...
        mSimIndicatorIcon = (ImageView) view.findViewById(R.id.simIndicatorIcon);
        ...
        // Add for plugin.
        ExtensionManager.getCallCardExt().onViewCreated(getActivity(), view);
        ExtensionManager.getRCSeCallCardExt().onViewCreated(getActivity(), view);
    }
...
//            }
    ...
    /// M: for VoLTE SS runtime indication @{
    // update for call waiting indication

    private static final int UPDATE_CALL_STATE_FOR_CALL_WAITING = 100;
    //add by zhanglongtao
    private static final int PHONE_VOICE_RECORDING = 200;

    private final Handler mHandler = new Handler() {
        @Override
        public void handleMessage(Message msg) {
            switch (msg.what) {
                case UPDATE_CALL_STATE_FOR_CALL_WAITING:
                    updateCallStateforCallWaiting();
                    break;
                /// @}
                /* add by zhanglongtao begin */
                case PHONE_VOICE_RECORDING:
                	if(getView() != null) {
                		String hms = Settings.System.getString(getView().getContext().getApplicationContext().getContentResolver(), "PHONE_VOICE_RECORDING_TIME");
                		mVoiceRecorderTimer.setText(hms);
                	} else {
                		stopTimer();
                	}
                	break;
                /* add by zhanglongtao begin */
                default:
                    Log.wtf(this, "mHandler: unexpected message: " + msg);
                    break;
            }
        }
    };

    private void updateCallStateforCallWaiting() {
        Log.d(this, "updateCallStateforCallWaiting~~");
        if (getPresenter().isCallWaiting()) {
            String dialingText = getResources().getString(R.string.card_title_dialing).toUpperCase();
            String indicationText = getResources().getString(R.string.volte_call_waiting_indication_text);
            // set for call waiting indications
            mCallStateLabel.setVisibility(View.VISIBLE);
            mCallStateLabel.setAllCaps(false);
            if (!mCallStateLabel.getText().equals(dialingText)) {
                mCallStateLabel.setText(dialingText);
            } else {
                mCallStateLabel.setText(indicationText);
            }
            mHandler.sendEmptyMessageDelayed(UPDATE_CALL_STATE_FOR_CALL_WAITING, 2000);
        } else {
            mCallStateLabel.setAllCaps(true);
            mHandler.removeMessages(UPDATE_CALL_STATE_FOR_CALL_WAITING);
        }
    }
    /// @}

    public void showVTCallUI(boolean show) {
        Log.d(this, "showVTCallUI()... show: " + show);
