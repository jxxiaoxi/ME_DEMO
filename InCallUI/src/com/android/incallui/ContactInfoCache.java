...
public class ContactInfoCache implements ContactsAsyncHelper.OnImageLoadCompleteListener {
    ...
    static ContactCacheEntry updateCallerInformation(Context context, Call call) {
        ContactCacheEntry entry = getInstance(context).getInfo(call.getCallId());
        final int numberPresentation = call.getIdentification().getNumberPresentation();

        /// M: for VoLTE normal call switch to ECC @{
        // if the call is marked as ECC by network, force build callerinfo as emergency
        if(FeatureOptionWrapper.isSupportVoLte() && call.isECCCall()) {
            Log.d(TAG, "updateCallerInformation, mark as ECC call~~");
            CallerInfo callerInfo = new CallerInfo();
            markAsEmergency(callerInfo, context);
            getInstance(context).findInfoQueryComplete(call.getIdentification(), callerInfo, call.isIncoming(), false);
            entry = getInstance(context).getInfo(call.getCallId());
        } else {
        /// @}
            if (entry != null) {
    ...
    private void findInfoQueryComplete(CallIdentification identification,
        ...
        if(FeatureOptionWrapper.isSupportVoLte()) {
            ...
            CallIdentification newIdentification = CallList.getInstance().getCall(identification.getCallId()).getIdentification();
            if (newIdentification != null) {
                identification = newIdentification;
                if (identification.isECCCall() && !callerInfo.isEmergencyNumber() && mContext != null) {
                    Log.d(TAG, "findInfoQueryComplete, mark as ECC call~~");
                    markAsEmergency(callerInfo, mContext);
//                    callerInfo.markAsEmergency(mContext);
        ...
        if(FeatureOptionWrapper.isSupportVoLte()) {
            cacheEntry.pau = identification.getPAUField();
        }
        /// @}
    ...
    public static void populateCacheEntry(Context context, CallerInfo info, ContactCacheEntry cce,
        ...
        if (FeatureOptionWrapper.isMtkPhoneNumberGeoDescription()) {
    ...
    /// M: for VoLTE normal call switch to ECC @{
    // if the call is marked as ECC by network, force build callerinfo as emergency
    static CallerInfo markAsEmergency(CallerInfo callerInfo, Context context) {
        callerInfo.phoneNumber = context.getString(R.string.emergency_call_dialog_number_for_display);
        callerInfo.photoResource = R.drawable.picture_emergency;
//        callerInfo.mIsEmergency = true;
        return callerInfo;
    }
    /// @}
}
