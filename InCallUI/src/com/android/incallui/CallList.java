...
public class CallList implements IInCallScreen {
    ...
    */
    ...
    /// @}
        ...
        if (updated) {
            /// M: update for incoming miss call @{
            if (FeatureOptionWrapper.isSupportDualTalk()) {
                Log.d(this, "update the incoming call list in onDisconnected.");
                updateIncomingCallList();
            }
            /// @}
    ...
    public void onIncoming(Call call, List<String> textMessages) {
        ...
        if (FeatureOptionWrapper.isSupportDualTalk()) {
            ...
            Log.d(this, "onIncoming (add to incoming call list) - " + call);
    ...
    public void onUpdate(List<Call> callsToUpdate) {
        ...
        for (Call call : callsToUpdate) {
            ...
            /// M: For support Dualtalk. @{
            // update mInComingCallList of dual talk before notify CallUpdateListener, see 01585653.
            if (FeatureOptionWrapper.isSupportDualTalk()) {
    ...
    public Call getSecondBackgroundCall() {
        ...
        if (FeatureOptionWrapper.isSupportDualTalk()
                ...
                && InCallPresenter.getInstance().mDualtalkCallInfo.getIsDualTalkMultipleHoldCase()) {
            call = getCdmaActiveCall();
        } else {
        /// @}
            call = getCallWithState(Call.State.ONHOLD, 1);
        }
        return call;
    }
    ...
    public void dumpCallList() {
        Log.d(this, "-----dumpCallList Begin------");
        if (mCallMap != null) {
            Log.d(this, "-----size of CallList : " + mCallMap.size());
            Iterator iterator = mCallMap.entrySet().iterator();
            while(iterator.hasNext()) {
                java.util.Map.Entry<Integer, Call> entry = (java.util.Map.Entry<Integer, Call>)iterator.next();
                Log.d(this, "-----Id / Call : " + entry.getKey() + " / " + entry.getValue());
            }
        }
        Log.d(this, "-----dumpCallList End------");
    }

    private void updateIncomingCallList() {
        Log.d(this, "enter updateIncomingCallList");
        dumpCallList();
        Iterator<Call> it = mInComingCallList.iterator();
        while (it.hasNext()) {
            Call call = it.next();
            Log.d(this, "current call = " + call);
            if (mCallMap.containsKey(call.getCallId())
                    /*&& getCall(call.getCallId()) == call*/
                    && (getCall(call.getCallId()).getState() == Call.State.INCOMING
                    || getCall(call.getCallId()).getState() == Call.State.CALL_WAITING)) {
                call.setRejectCallNotifyMsg(getCall(call.getCallId()).getRejectCallNotifyMsg());
                Log.d(this, "updateIncomingCallList  - ", call);
            } else {
        ...
        Log.d(this, "exit updateIncomingCallList");
    }
    ...
    public void switchIncomingCalls() {
        ...
        // trigger to update answerFragment.
        if (getIncomingCall() != null) {
            for (Listener listener : mListeners) {
                listener.onIncomingCall(getIncomingCall());
            }
        }
    }
}
