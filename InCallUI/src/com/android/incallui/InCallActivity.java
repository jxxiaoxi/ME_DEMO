...
import com.android.incallui.CallButtonFragment.OnCallButtomTouchScreenListener;
import com.android.incallui.DialpadFragment.OnDiapadListener;
...
import com.mediatek.incallui.volte.VoLteConfManagerFragment;
 ...
 */
public class InCallActivity extends Activity implements OnCallButtomTouchScreenListener ,OnDiapadListener{
    ...
    protected void onCreate(Bundle icicle) {
        ...
        */
		...
		if (SystemProperties.getBoolean("ro.phone.lockscreen_for_yl", false)) {
			mSlectLockForYL = Settings.System.getInt(this.getContentResolver(),
					"other_lock_key", 1);
		...
		Log.e("huiye", "onCreate   isPhoneLock: " + isPhoneLock
				+ "  ;mSlectLockForYL : " + mSlectLockForYL);
    ...
    protected void onResume() {
		...
		// add by liuwei for 5s srceen off
		Log.e("huiye", "**********InCallActivity onResume  mIsOnCalling  : "
				+ mIsOnCalling + " isPhoneLock : " + isPhoneLock);
		isActivityOnPause = false;
		if (isPhoneLock && mIsOnCalling) {
			if (mCallButtonFragment != null) {
				mCallButtonFragment.isPhoneInBackground = false;
			}
        ...
        if(InCallPresenter.getInstance().getInCallState().isIncoming()){ 
        	 CallCommandClient.getInstance().inCallActivityShowDone(); 
    	}else{ 
    	 Log.d(this, "CallState != INCOMING"); 
    	}
    }
    ...
    protected void onPause() {
        ...
        super.onPause();
		// add by liuwei for 5s srceen off
		Log.e("huiye", "**************onPause   isPhoneLock = " + isPhoneLock
				+ "  ;mTimerTask = " + mTimerTask);
		isActivityOnPause = true;
		if (isPhoneLock && mTimerTask != null) {
			mTimerTask.cancel();
			if (mCallButtonFragment != null) {
				mCallButtonFragment.isPhoneInBackground = true;
			}
		}
    ...
    protected void onDestroy() {
		...
		// add by liuwei for 5s srceen off
		Log.e("huiye", "onDestroy   mTimerTask: " + mTimerTask);
		if (isPhoneLock && mTimerTask != null) {
			mTimerTask.cancel();
		}
    ...
    public void finish() {
        ...
        /// @}
        //Fix bug 8255:对方挂断时，一直处于短信拒接界面
        dismissPendingDialogsAndHideConferenceFragment();
    ...
    public void onBackPressed() {
        ...
        } else if (FeatureOptionWrapper.isSupportVoLte() && mVoLteConfManagerFragment != null && mVoLteConfManagerFragment.isVisible()) {
            mVoLteConfManagerFragment.setVisible(false);
            return;
        /// @}
        }
    ...
    public void dismissPendingDialogs() {
        ...
        if(FeatureOptionWrapper.isSupportVoLte()) {
            VoLteConfUiController.getInstance().dismissAddConferenceMemberDialog();
        }
        /// @}
    }
    ...
    private void initializeInCallMTK() {
        ...
        /// M: for VoLTE Conference Call @{
        if (FeatureOptionWrapper.isSupportVoLte() && mVoLteConfManagerFragment == null) {
            mVoLteConfManagerFragment = (VoLteConfManagerFragment) getFragmentManager()
                    .findFragmentById(R.id.voLteConfManagerFragment);
            mVoLteConfManagerFragment.getView().setVisibility(View.INVISIBLE);
        }
        /// @}
    }
...
//fix:进行中的通话被对方挂后，来电界面的菜单不会自动隐藏.
    ...
    private void setOrientation() {
        if(FeatureOptionWrapper.isMtkSmartBookSupport() || FeatureOptionWrapper.isMtkHdmiSupport()) {
            Log.d(this, "MTK_SMARTBOOK_SUPPORT / MTK_HDMI_SUPPORT: " + FeatureOptionWrapper.isMtkSmartBookSupport() + " / "
                    + FeatureOptionWrapper.isMtkHdmiSupport());
        ...
        if (!isFinishing()) {
            ...
            if (!isIncoming) {
                ...
                if (resId != INVALID_RES_ID) {
                    ...
                    if (isForegroundActivity()
                            && !VTSettingLocal.getInstance().mAutoDropBack) {
    ...
    /// M: for VoLTE Conference Call @{
    private VoLteConfManagerFragment mVoLteConfManagerFragment;
    public void displayVoLteManageConfPanel(boolean showPanel) {
        if (showPanel && mVoLteConfManagerFragment != null) {
            mVoLteConfManagerFragment.setVisible(true);
        }
    }
    /// @}
	...
	Handler handler = new Handler() {
		...
		public void handleMessage(Message msg) {
			...
			case 1:
				...
				intent.setAction("phone_go_to_sleep");
	...
	public void startLockWindowTimer() {
		Log.e("huiye", "InCallActivity startLockWindowTimer"
				+ "  ; isActivityOnPause " + isActivityOnPause);
		if (mTimer != null) {
			if (mTimerTask != null) {
				mTimerTask.cancel();
			}
	...
	public void onCalling(boolean enabled) {
		...
		Log.e("huiye", " InCallActivity onCalling  : " + enabled);
		mIsOnCalling = enabled;
		if (enabled) {
			startLockWindowTimer();
		} else {
			if (mTimerTask != null) {
	...
	public void onTouchScreen() {
		// TODO Auto-generated method stub
		Log.e("huiye", "InCallActivity onTouchScreen ");
		startLockWindowTimer();
	}
	...
	public void isLockOrScreenOff(boolean enabled) {
		// TODO Auto-generated method stub
		Log.e("huiye", "InCallActivity isLockOrScreenOff  " + enabled);
		if (enabled == false) {
			startLockWindowTimer();
		}
	}
	...
	@Override
	public void onDiapadClickOrTouch() {
		// TODO Auto-generated method stub
		Log.e("huiye", "InCallActivity onDiapadClickOrTouch");
		startLockWindowTimer();
	}
}
