...
public class InCallPresenter implements CallList.Listener {
    ...
    public void setUp(Context context, CallList callList, AudioModeProvider audioModeProvider) {
        ...
        if (FeatureOptionWrapper.isSupportVoLte()) {
            mVoLteConfUiController = VoLteConfUiController.getInstance();
            mVoLteConfUiController.init(context);
            addListener(mVoLteConfUiController);
        }
        /// @}
...
//0011862拨号界面输入错误的超长号码，可拨出无任何提示，拨出界面直接出现通话结束
        boolean show = false;
     try{
        if(mInCallActivity != null && mInCallActivity.mCallCardFragment != null){
            show = mInCallActivity.mCallCardFragment.getCallName().length() >= 25
                  && mInCallActivity.mCallCardFragment.getCallNumber().length() == 0; //联系入外
            show |= mInCallActivity.mCallCardFragment.getCallName().length() > 0
                  && mInCallActivity.mCallCardFragment.getCallNumber().length() >= 25; //联系人内
            Log.d("huiye","huiye---name="+  mInCallActivity.mCallCardFragment.getCallName() );
            Log.d("huiye","huiye---number="+  mInCallActivity.mCallCardFragment.getCallNumber() );
        }
     }catch(Exception e){
		e.printStackTrace();
     }
        if(show)
             Toast.makeText(mContext, mContext.getResources().getString(R.string.error_number), Toast.LENGTH_LONG).show();
...
//0012765 结束电话振动
        if (SystemProperties.getBoolean("ro.finishcall.vibrator", false) && mVibrator == null) {
...
//fix:进行中的通话被对方挂后，来电界面的菜单不会自动隐藏.
    ...
    private void startUi(InCallState inCallState) {
        ...
        /// M: ALPS01565336. If incoming call type is different from active or hold call,
        /// the state is not CALL_WAITING. @{
        final boolean hasIncomingCall = (incomingCall != null);
        final boolean hasActiveOrHoldCall = (mCallList.getActiveOrBackgroundCall() != null);
        /// @}
        ...
        if (mProximitySensor.isScreenReallyOff() /*&& isCallWaiting*/
                && hasIncomingCall && hasActiveOrHoldCall) {
            if (isActivityStarted()) {
                mInCallActivity.finish();
            }
            mInCallActivity = null;
    ...
    private void attemptCleanup() {
        ...
        if (shouldCleanup) {
            ...
            /// M: for VoLTE Conference Call @{
            if(FeatureOptionWrapper.isSupportVoLte() && mVoLteConfUiController != null) {
                removeListener(mVoLteConfUiController);
                mVoLteConfUiController = null;
            }
            /// @}
 ...
 // true for checking, false for recording
    ...
    public void showInCallMTK() {
        if (mContext == null) {
            Log.e(this, "showInCallMTK()...mContext is null, do nothing !");
            return;
        }
    ...
    /// M: for VoLTE Conference Call @{
    private VoLteConfUiController mVoLteConfUiController;
    /// @}
}
