...
public class SearchDropTargetBar extends FrameLayout implements DragController.DragListener {
    ...
    private void setupAnimation(ObjectAnimator anim, final View v) {
		...
		if(Launcher.HIDE_SEARCH_BAR && v == mDropTargetBar) {
			final int[] pos = new int[2];
            		v.getLocationOnScreen(pos);
			if(pos[1] != 0 && mLauncher.getAdjustHeight() != 0) {
				mLauncher.getWorkspace().setPadding(mLauncher.getWorkspace().getPaddingLeft(), 
					mLauncher.getWorkspace().getPaddingTop() - mLauncher.getAdjustHeight(), 
					mLauncher.getWorkspace().getPaddingRight(), 
					mLauncher.getWorkspace().getPaddingBottom());
				mLauncher.setAdjustHeight(0);
