...
if(Launcher.HIDE_SEARCH_BAR && isVisible && !isAllAppsApplication(source, info)) {
	Rect rect= new Rect();
        mLauncher.getWindow().getDecorView().getWindowVisibleDisplayFrame(rect);
        int statusBarHeight = rect.top;

	mLauncher.setAdjustHeight(statusBarHeight);
	mLauncher.getWorkspace().setPadding(mLauncher.getWorkspace().getPaddingLeft(), 
		mLauncher.getWorkspace().getPaddingTop() + statusBarHeight, 
