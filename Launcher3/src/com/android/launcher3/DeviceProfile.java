...
public class DeviceProfile {
    ...
    DeviceProfile(Context context,
        ...
        if (4 == SystemProperties.getInt("ro.launcher.defaultworkspace", -1))
            numColumns = 4;

        // Snap to the closest hotseat size
        numHotseatIcons = closestProfile.numHotseatIcons;
        allAppsrank=SystemProperties.getInt("ro.launcher.allappsrank", -1);
        if(allAppsrank==-1)
        {	
        ...
        {	
        hotseatAllAppsRank = allAppsrank;
        }
    ...
    private void updateIconSize(float scale, int drawablePadding, Resources resources,
        ...
        if (Launcher.HIDE_SEARCH_BAR) {
        searchBarHeightPx = resources.getDimensionPixelSize(R.dimen.dynamic_grid_search_bar_height_no_search_bar);
        } else {
        ...
        // Hotseat
        if(SystemProperties.getBoolean("ro.zxbh",false)){
            hotseatBarHeightPx = iconSizePx + 4 * edgeMarginPx + (int) Math.ceil(fm.bottom - fm.top);
            hotseatCellWidthPx = iconSizePx;
            hotseatCellHeightPx = iconSizePx + (int) Math.ceil(fm.bottom - fm.top);
    	} else {
        ...
        if (orientation == CellLayout.LANDSCAPE &&
            ...
            if (isTablet()) {
                ...
                padding.set(desiredWorkspaceLeftRightMarginPx - defaultWidgetPadding.left,
                        Launcher.HIDE_SEARCH_BAR ? edgeMarginPx : searchBarBounds.bottom,
    ...
    public void layout(Launcher launcher) {
        ...
        if (hasVerticalBarLayout) {
            ...
            searchBar.setPadding(
                    2 * edgeMarginPx,
                    Launcher.HIDE_SEARCH_BAR ? 0 : getSearchBarTopOffset(),
                    2 * edgeMarginPx, 0);
        }
