...
public class NetworkSettingList extends PreferenceActivity
    ...
    private INetworkSettingExt mExt;
    ...
    protected void onCreate(Bundle icicle) {
        ...
        mExt = ExtensionManager.getNetworkSettingExt();
        geminiPhoneInit();
        if (DBG) {
            log("It's a GeminiPhone ? = " + GeminiUtils.isGeminiSupport() + "SIM_ID = " + mSlotId);
        }
        ...
        i.putExtra(PhoneConstants.GEMINI_SIM_ID_KEY, mSlotId);
        /// @}
        startService(i);
    ...
    public boolean onPreferenceTreeClick(PreferenceScreen preferenceScreen, Preference preference) {
        ...
        if (mExt.onPreferenceTreeClick(mNetworkMap.get(selectedCarrier), mSlotId)) {
            return true;
        }
        Message msg = mHandler.obtainMessage(EVENT_NETWORK_SELECTION_DONE);
        /// M: add for gemini phone @{
        PhoneWrapper.selectNetworkManually(mPhone, mNetworkMap.get(selectedCarrier), msg, mSlotId);
        /// @}
    ...
    private void networksListLoaded(List<OperatorInfo> result, int status) {
        ...
        result = mExt.customizeNetworkList(result, mSlotId);
        /** M: ALPS01564316 In extreme case, NetworkQueryService started after onPause,
         *  so don't show anything if in background @{*/
        if (!mIsForeground) {
            log("networkslist activity in background, just retrun");
            return;
        }
        /** @}*/
