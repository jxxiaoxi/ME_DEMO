...
public class FdnSetting2 extends  PreferenceActivity
    ...
    private void displayMessage(int strId) {
        ...
        if (strId == R.string.puk2_requested) {
            text = ExtensionManager.getPhoneMiscExt().changeTextContainingSim(text, mSlotId);
        }
    ...
    private void updateFDNPreference() {
        ...
        if (getRetryPin2Count() == 0) {
            ...
            if (getRetryPuk2Count() == 0) {
                textMsg = ext.changeTextContainingSim(getString(R.string.sim_permanently_locked), mSlotId);
            } else {
                textMsg = ext.changeTextContainingSim(getString(R.string.fdn_enable_puk2_requested), mSlotId);
            }
