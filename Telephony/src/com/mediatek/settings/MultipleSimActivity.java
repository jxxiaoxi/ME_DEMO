    ...
    protected void setPreference(SimCardInfoPreference p) {
        ...
        if (mType == WidgetType.CheckBox) {
            ...
            if (GeminiUtils.isTddSupportForLte()) {
                log(":::MultipleSimActivity setPreference get tdd no init value");				
            } else {
                p.setValue(sp.getString(key, "0"));
            }
            if (NETWORK_MODE_NAME.equals(mFeatureName)) {
                ExtensionManager.getMobileNetworkSettingsExt().setPreferredNetworkModeEntriesAndValues((ListPreference)p);
                int settingsNetworkMode = android.provider.Settings.Global.getInt(
                    mPhone.getContext().getContentResolver(), getNetworkModeName(p.getSimSlotId()), 0);
                log(":::MultipleSimActivity setPreference get settingsNetworkMode first value = " + settingsNetworkMode);
                /// M: add for wcdma prefer feature
                if (settingsNetworkMode == Phone.NT_MODE_GSM_UMTS){
                    settingsNetworkMode = Phone.NT_MODE_WCDMA_PREF;
                }

                //for sglte we also only cover:
                //Phone.NT_MODE_GSM_UMTS is 3 but we think is NT_MODE_WCDMA_PREF
                //Phone.NT_MODE_WCDMA_PREF is 0
                //Phone.NT_MODE_LTE_GSM_WCDMA
                log(":::MultipleSimActivity setPreference settingsNetworkMode init value = " + settingsNetworkMode);
                if (GeminiUtils.isTddSupportForLte()) {
                    if (settingsNetworkMode != Phone.NT_MODE_WCDMA_PREF &&
                        settingsNetworkMode != Phone.NT_MODE_LTE_GSM_WCDMA) {
                        log(":::MultipleSimActivity setPreference the getting provider value is not excepted!");						
                        return;
                    }
                }
