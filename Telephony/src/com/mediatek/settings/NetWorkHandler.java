...
public class NetWorkHandler extends Handler {
    ...
    private void handleGetPreferredNetworkTypeResponse(Message msg) {
        ...
        if (ar.exception == null) {
            ...
            if (modemNetworkMode == Phone.NT_MODE_WCDMA_PREF
                ...
                if (modemNetworkMode == Phone.NT_MODE_GSM_UMTS) {
                    ...
                    if (settingsNetworkMode != Phone.NT_MODE_WCDMA_PREF) {
                        ...
                        android.provider.Settings.Global.putInt(
                                ...
                                android.provider.Settings.Global.PREFERRED_NETWORK_MODE,
                                settingsNetworkMode);
                        PhoneLog.d(TAG, "handleGetPreferredNetworkTypeResponse: if diff update when get3 with 0 : settingsNetworkMode = "
                            + settingsNetworkMode);
                    }
                } else {
                    if (modemNetworkMode != settingsNetworkMode) {
                        ...
                        android.provider.Settings.Global.putInt(
                                ...
                                android.provider.Settings.Global.PREFERRED_NETWORK_MODE,
                                settingsNetworkMode);
                        PhoneLog.d(TAG, "handleGetPreferredNetworkTypeResponse: modem and setting provider not same: settingsNetworkMode = "
                            + settingsNetworkMode);
                    }
                }
                //for sglte if not the need mode the list preference not to set any selected
                if (GeminiUtils.isLteSupport() && GeminiUtils.isTddSupportForLte()) {
                    PhoneLog.d(TAG, "NetWorkHandler MTK_LTE_DC_SUPPORT support update modemNetworkMode:" + modemNetworkMode);
                    GeminiUtils.updateSglteMode(mPreference, modemNetworkMode);
                } else {
                    ...
                    //tdd csfb
                    ExtensionManager.getMobileNetworkSettingsExt().updateModeSummary(mPreference);
                }
                ...
                if (GeminiUtils.isLteSupport() && GeminiUtils.isTddSupportForLte()) {
                    PhoneLog.d(TAG, "handleGetPreferredNetworkTypeResponse: else: for sglte no need to reset");
                } else {
            ...
            PhoneLog.d(TAG, "handleGetPreferredNetworkTypeResponse: exception" + ar.exception);
        }
    }
    ...
    private void handleSetPreferredNetworkTypeResponse(Message msg) {
        ...
        if (ar.exception != null) {
            /// for 4G switch @{
            int slotId = GeminiUtils.get34GCapabilitySIM();
            /// @}
            PhoneWrapper.getPreferredNetworkType(mPhone, obtainMessage(MESSAGE_GET_PREFERRED_NETWORK_TYPE), slotId);
        } else {//just for save manual mode
//            int networkMode = Integer.valueOf(mPreference.getValue()).intValue();
//            PhoneLog.d(TAG, "handleSetPreferredNetworkTypeResponse: no exception and save the manual provider:" + networkMode);
...
//                networkMode);
    ...
    public void setPreference(ListPreference preference) {
        PhoneLog.d(TAG, "setPreference: getEntry = " + preference.getEntry());
        mPreference = preference;
    }
}
