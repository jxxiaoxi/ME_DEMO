...
public class VTManager {
    ...
    public int startRecording(int type, long maxSize) {
        ...
        if (!sampleDir.canWrite()) {
            ...
            mRecordedFilePath = cameraImageBucketName + "/Recording/" + mRecordedFileName
                    + ".3gp";
            Log.i(TAG, "recordedFileName is: " + mRecordedFilePath);

            // To create file's directory
            File recordDirectory = new File(cameraImageBucketName + "/Recording/");
            recordDirectory.mkdirs();
        } else {
            sampleDir = new File(sampleDir.getAbsolutePath() + "/Recording");
            if (!sampleDir.exists()) {
                sampleDir.mkdirs();
            }
