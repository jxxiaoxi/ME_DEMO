...
public final class VTCallUtils {
    ...
    /**
     * check VT image files when Phone process is created, create if no files
    ...
    /**
     * Check default picture, including default picture to replace peer and default picture to replace local.
     */
    ...
    /**
     * Check user selected picture on certain slot, including picture to replace peer and picture to replace local.
     * @param slotId
     */
    ...
    public static int placeVTCall(Phone phone, String number, Uri contactRef, int slotId) {
        ...
        try {
            ...
            /// M: For ALPS01547377 @{
            // For quick dial while previous VT Call is in disconnecting state,
            // We have not done clean-operation. Then abandon this VT-dial request.
            if (VTManagerWrapper.getInstance().getState() != VTManager.State.CLOSE) {
                if (DBG) {
                    log("VTManager is not in CLOSE state, abandon this VT-dial request!");
                }
                return Constants.CALL_STATUS_FAILED;
            }
            ///@}
        ...
        if (status != Constants.CALL_STATUS_FAILED) {
            ...
            VTSettingUtils.getInstance().setSlotId(slotId);

            // update VTInCallScreenFlags.
            VTCallFlags.getInstance().reset();
            VTCallFlags.getInstance().mVTIsMT = false;

            VTCallUtils.notifyVTResInUse(true);
    ...
    public static void internalAnwerVTCallPre() {
        ...
        VTCallFlags.getInstance().reset();
        VTCallFlags.getInstance().mVTIsMT = true;
        ...
        if (isVTActive()) {
            ...
            if (DBG) {
                log("anwerVTCallPre:"
                    + " set VTCallFlags.getInstance().mVTShouldCloseVTManager = false");
            }
            VTCallFlags.getInstance().mVTShouldCloseVTManager = false;
        }
        ...
        if (null != simInfo) {
            VTSettingUtils.getInstance().setSlotId(simInfo.mSimSlotId);
        } else {
    ...
    public static void closeVTManager() {
        ...
        VTCallUtils.notifyVTResInUse(false);
    }
    ...
    public static boolean isVTIdle() {
        ...
        List<Call> fgCalls = PhoneGlobals.getInstance().mCM.getForegroundCalls();
        List<Call> bgCalls = PhoneGlobals.getInstance().mCM.getBackgroundCalls();
        List<Call> ringingCalls = PhoneGlobals.getInstance().mCM.getRingingCalls();

        for (Call call : ringingCalls) {
            if (call.getState() != Call.State.IDLE && call.getLatestConnection().isVideo()) {
                return false;
            }
        }

        for (Call call : fgCalls) {
            if (call.getState() != Call.State.IDLE && call.getLatestConnection().isVideo()) {
                return false;
            }
        }

        for (Call call : bgCalls) {
            if (call.getState() != Call.State.IDLE && call.getLatestConnection().isVideo()) {
                return false;
            }
        }
     ...
     */
    public static void setLocalViewToVTManager() {
        PhoneLog.d(LOG_TAG, "setLocalViewToVTManager()... isMT: " + VTCallFlags.getInstance().mVTIsMT);

        if (VTCallFlags.getInstance().mVTIsMT) {
            String showLocalMT = VTSettingUtils.getInstance().getShowLocalMT();
            PhoneLog.d(LOG_TAG, "setLocalViewToVTManager: MT = " + showLocalMT);
            // Video Call Settings -> Video Incoming Call (0-video, 1-ask, 2-picture)
            if (showLocalMT.equals("0")) {
                VTManagerWrapper.getInstance().setLocalView(0, "");
            } else if (showLocalMT.equals("1")) {
                // Show VTInCallScreen#mVTMTAsker, Before user select display video, just show picture at local.
                updatePicToReplaceLocalVideo();
            } else {
                updatePicToReplaceLocalVideo();
            }
        } else {
            boolean showLocalMO = VTSettingUtils.getInstance().getShowLocalMO();
            PhoneLog.d(LOG_TAG, "setLocalViewToVTManager: MO = " + showLocalMO);
    ...
    public static void setPeerViewToVTManager() {
        boolean replacePeer = VTSettingUtils.getInstance().getToReplacePeer();
        String picToReplacePeer = VTSettingUtils.getInstance().getPicToReplacePeer();
        PhoneLog.d(LOG_TAG, "setPeerViewToVTManager()... mToReplacePeer / mPicToReplacePeer: "
                + replacePeer + " / " + picToReplacePeer);
        if (replacePeer) {
            if (picToReplacePeer.equals(VTAdvancedSetting.SELECT_DEFAULT_PICTURE2)) {
                VTManagerWrapper.getInstance().setPeerView(1, VTAdvancedSetting.getPicPathDefault2());
            }else {
                VTManagerWrapper.getInstance().setPeerView(1, VTAdvancedSetting.getPicPathUserselect2(VTSettingUtils.getInstance().getSlotId()));
            }
        } else {
            VTManagerWrapper.getInstance().setPeerView(0, "");
        }
    }
    ...
    public static void updatePicToReplaceLocalVideo() {
        String picToReplaceLocal = VTSettingUtils.getInstance().getPicToReplaceLocal();
        PhoneLog.d(LOG_TAG, "updatePicToReplaceLocalVideo: " + picToReplaceLocal);
        if (picToReplaceLocal.equals("2")) { // my setting picture
            VTManagerWrapper.getInstance().setLocalView(
                    1, VTAdvancedSetting.getPicPathUserselect(VTSettingUtils.getInstance().getSlotId()));
        } else if (picToReplaceLocal.equals("1")) { // freeze me picture.
...
/**
    ...
    /**
     * To check current network on the device supports VT(3G) or not.
     * @param slotId
     * @return true for support, false for not support.
     */
    public static boolean isUnder3GService(int slotId) {
        boolean result = true;
        int csNetType = getCSNetType(slotId);
        if ((0 == csNetType) || (1 == csNetType) || (2 == csNetType)) {
            result = false;
        }
        return result;
    }
    ...
    /**
     * Check the setting is auto drop back or not for VT based on certain slot.
     * @param slotId
     * @return
     */
    public static boolean isAutoDropBack(int slotId) {
        ...
        if (sp != null) {
            result = sp.getBoolean("button_vt_auto_dropback_key_" + slotId, false);
        } else {
    ...
    /**
     * Handle the case of vt call auto drop back. If the local is out of 3G Service and AutoDropBack in settings is true.
     *
    ...
    public static boolean handleVTCallAutoDropBack(int slotId, Intent intent) {
        if ( !isUnder3GService(slotId) && isAutoDropBack(slotId)) {
            log("handleVTCallAutoDropBack()...just auto drop back !");
    ...
    public static void notifyVTResInUse(boolean inUse) {
        Intent intent = new Intent();
        if (inUse) {
            intent.setAction(VTCallUtils.VT_CALL_START);
        } else {
            intent.setAction(VTCallUtils.VT_CALL_END);
        }
