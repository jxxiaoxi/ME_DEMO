...
public class CallManagerWrapper {
     ...
     * @return Connection
     */
    public static Connection dial(Phone phone, String numberToDial, int slotId, boolean isIMSCallOnly) throws CallStateException {
        Assert.assertNotNull(phone);
        ...
        try {
            ...
            PhoneLog.d(TAG, "dial, number = " + numberToDial + ", slotId = " + dialSlot
                    + ", isIMSOnly = " + isIMSCallOnly);
            if (!isIMSCallOnly) {
                connection = CallManager.getInstance().dial(
                        PhoneWrapper.getPhoneBySlotId(phone, dialSlot), numberToDial);
            } else {
                /// M: for VoLTE IMS call @{
//                connection = CallManager.getInstance().dialWithSipUri(PhoneWrapper.getPhoneBySlotId(phone, dialSlot), numberToDial);  // mark volte part
                /// @}
            }
    ...
    public static void registerForVoLteConferenceCallNotification(Handler handler, int what,
            Object obj) {
//        CallManager.getInstance().registerForVoLteConferenceCallNotification(handler, what, obj);     // mark volte part
    }

    public static void unregisterForVoLteConferenceCallNotification(Handler handler) {
//        CallManager.getInstance().unregisterForVoLteConferenceCallNotification(handler);  // mark volte part
    }

    public static void addConferenceMember(Phone phone, String number) {
//        PhoneLog.d(TAG, "[addConferenceMember], number = " + number);
//        try {
////            CallManager.getInstance().addConferenceMember(phone, number);     // mark volte part
//        } catch (CallStateException e) {
//            PhoneLog.e(TAG, "[addConferenceMember], exception find!!!~~");
//        }
    }
    /// @}
}
