...
public class PhoneInterfaceManagerEx extends ITelephonyEx.Stub {
    ...
    /* SIM switch start */
    private ArrayList<Integer> mSimSwitchLocks = new ArrayList<Integer>();
    private static int mSimSwitchLockCounter;
    /* SIM switch end */
        ...
        public void handleMessage(Message msg) {
            ...
            switch (msg.what) {
               ...
               case CMD_SIM_IO:
                    ...
                    if (PhoneFactory.isGeminiSupport() == true) {
    ...
    public Bundle queryNetworkLock(int category, int simId) {
        ...
        if(false == PhoneFactory.isGeminiSupport()) {
    ...
    */ 
    ...
    public boolean isAirplanemodeAvailableNow() {
        if(PhoneFactory.isGeminiSupport() == true) {
            return !((GeminiPhone)mPhone).isSwitchingCapabilitySim();
        } else {
            return true;
        }
    }
    ...
    */ 
    ...
    public int get3GCapabilitySIM() {
        return getCapabilitySIM(TelephonyCapabilities.CAPABILITY_3G);
    }
    ...
    public boolean set3GCapabilitySIM(int simId) {
        return setCapabilitySIM(TelephonyCapabilities.CAPABILITY_3G, simId);
    }
    ...
    public boolean release3GSwitchLock(int lockId) {
        return releaseSimSwitchLock(lockId);
    }
    ...
    public boolean is3GSwitchManualEnabled() {
        return isSimSwitchManualModeEnabled();
    }
    ...
    public boolean is3GSwitchManualChange3GAllowed() {
        return isSimSwitchManualChangeSlotAllowed();
    }
    ...
    public int get3GSwitchAllowed3GSlots() {
        return getSimSwitchAllowedSlots();
    }
/* 3G switch end */
...
/* SIM switch start */
     ...
     * @internal
     */
    public int getCapabilitySIM(int capability) {
        if (GeminiUtils.isGeminiSupport() && PhoneFactory.isSimSwitchSupport()) {
            return ((GeminiPhone) mPhone).getCapabilitySIM(capability);
        } 
        else if (GeminiUtils.isGeminiSupport() && PhoneFactory.is3GSwitchSupport()) {
     ...
     * 
     */
    public int getHsCapabilitySIM() {
        int capability = (PhoneFactory.isLteSupport() ? TelephonyCapabilities.CAPABILITY_34G : TelephonyCapabilities.CAPABILITY_3G);
        return getCapabilitySIM(capability);
    }
     ...
     * @internal
     */
    public boolean setCapabilitySIM(int capability, int simId) {
        boolean result = false;
        if (mSimSwitchLocks.isEmpty()) {
            PhoneConstants.State state = mCM.getState();
            if (state == PhoneConstants.State.IDLE && PhoneFactory.isSimSwitchSupport()) {
                result = ((GeminiPhone)mPhone).setCapabilitySIM(capability, simId);
            } 
            else if (state == PhoneConstants.State.IDLE && PhoneFactory.is3GSwitchSupport()) {
                result = ((GeminiPhone)mPhone).set3GCapabilitySIM(simId);
            }
            else
                Log.w(LOG_TAG, "Phone is not idle, cannot do switch ["+capability+"][" + state + "]");
        } else {
            Log.w(LOG_TAG, "sim switch locked, cannot do switch ["+capability+"][" + mSimSwitchLocks + "]");
        }
        return result;
    }
     ...
     * @internal
     */
    public int aquireSimSwitchLock() {
        Integer lock = new Integer(mSimSwitchLockCounter++);
        mSimSwitchLocks.add(lock);
        
        Intent intent = new Intent(TelephonyIntents.EVENT_SIM_SWITCH_LOCK_CHANGED); // it was TelephonyIntents.EVENT_3G_SWITCH_LOCK_CHANGED
        intent.putExtra(TelephonyIntents.EXTRA_SIM_SWITCH_LOCKED, true); // it was TelephonyIntents.EXTRA_3G_SWITCH_LOCKED
        mApp.getApplicationContext().sendBroadcast(intent);
        
        Log.i(LOG_TAG, "aquire Switch lock: " + lock);
        return lock;
    }
     ...
     * @internal
     */
    public boolean releaseSimSwitchLock(int lockId) {
        boolean result = false;
        int index = 0;
        Iterator<Integer> it = mSimSwitchLocks.iterator();
        while (it.hasNext()) {
            int storedLockId = it.next();
            if (storedLockId == lockId) {
                int removedLockId = mSimSwitchLocks.remove(index);
                result = (lockId == removedLockId);
                Log.i(LOG_TAG, "removed Switch lockId: " + removedLockId + "[" + lockId + "]");

                Intent intent = new Intent(TelephonyIntents.EVENT_SIM_SWITCH_LOCK_CHANGED); // it was TelephonyIntents.EVENT_3G_SWITCH_LOCK_CHANGED
                intent.putExtra(TelephonyIntents.EXTRA_SIM_SWITCH_LOCKED, !mSimSwitchLocks.isEmpty()); // it was TelephonyIntents.EXTRA_3G_SWITCH_LOCKED
                mApp.getApplicationContext().sendBroadcast(intent);
                break;
            }
            ++index;
     ...
     * @internal
     */
    public boolean isSimSwitchLocked() {
        return !mSimSwitchLocks.isEmpty();
    }
     ...
     * @internal
     */
    public boolean isSimSwitchManualModeEnabled() {
        if (PhoneFactory.isSimSwitchSupport()) {
            return ((GeminiPhone)mPhone).isSimSwitchManualModeEnabled();
        } else if (PhoneFactory.is3GSwitchSupport()) {
     ...
     * @internal
     */
    public boolean isSimSwitchManualChangeSlotAllowed() {
        if (PhoneFactory.isSimSwitchSupport()) {
            return ((GeminiPhone)mPhone).isSimSwitchManualChangeSlotAllowed();
        } else if (PhoneFactory.is3GSwitchSupport()) {
     ...
     * @internal
     */
    public int getSimSwitchAllowedSlots() {
        if (PhoneFactory.isSimSwitchSupport()) {
            return ((GeminiPhone)mPhone).getSimSwitchAllowedSlots();
        } else if (PhoneFactory.is3GSwitchSupport()) {
...
/* SIM switch end */
    ...
    */ 
    ...
    public int getNetworkType(int simId) {
        //[ALPS01620689]-ADD : Start        
        //return ((GeminiPhone)mPhone).getPhonebyId(simId).getServiceState().getNetworkType();		
        int nwType = ((GeminiPhone)mPhone).getPhonebyId(simId).getServiceState().getDataNetworkType();
        if (nwType != ServiceState.RIL_RADIO_TECHNOLOGY_UNKNOWN){
            return nwType;
        } else {
            return ((GeminiPhone)mPhone).getPhonebyId(simId).getServiceState().getVoiceNetworkType();
        }
    }  

    public int getDataNetworkType(int simId) {                
        return ((GeminiPhone)mPhone).getPhonebyId(simId).getServiceState().getDataNetworkType();		
    }  
    ...
    public int dataConnectivityDecideSimId(){
        if (GeminiUtils.isGeminiSupport()) {
            return ((GeminiPhone)mPhone).dataConnectivityDecideSimId();
        } else {
            log("Single SIM");
            return 0;
        }
    ...
    public void setDataOnToMD(boolean enable){
        log("Set Data On/Off To MD:" + enable);
        if (GeminiUtils.isGeminiSupport()) {            
            ((GeminiPhone)mPhone).setDataOnToMD(enable);
        } else {
            mPhone.setDataOnToMD(enable);
        }
    }    
  ...
  /**
     ...
     * @internal 
     */
    public String getLocatedPlmn(int simId){
        if (GeminiUtils.isGeminiSupport()) {
            return ((GeminiPhone)mPhone).getPhonebyId(simId).getLocatedPlmn();
        } else {
            return ((GSMPhone)mPhone).getLocatedPlmn();
        }		
    }		
