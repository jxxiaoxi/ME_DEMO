...
public class SuppMessageManager {
    ...
    public void onSuppServiceNotification(AsyncResult r) {
        ...
        if (notification.notificationType == 0) {
            switch (notification.code) {
                ...
                // if the call is waiting, update UI as waiting state
                        /*// mark volte part
                case SuppServiceNotification.MO_CODE_CALL_IS_WAITING:
                    if(FeatureOption.MTK_VOLTE_SUPPORT) {
                        log("the MO call is waiting~~");
                        PhoneGlobals.getInstance().getCallModeler().markDialingCalltoWaiting();
                ...
                // In VoLTE, the number dialed may be judged as emergency number by network,
                // then we should switch the screen as ECC
                    /*// mark volte part
                case SuppServiceNotification.MO_CODE_CALL_IS_EMERGENCY:
                    if(FeatureOption.MTK_VOLTE_SUPPORT) {
                        log("the MO call is a Emerngecy call~~");
                        PhoneGlobals.getInstance().getCallModeler().markDialingCallAsECC();
                        PhoneGlobals.getInstance().getCallModeler().updateCalls();
                    }
                    break;
                    */
                ...
                default:
                    showSSNotificationToast(notification);
                    break;
            }
    ...
    private void showSSNotificationToast(SuppServiceNotification notification) {
        String msg = null;
        // MO
        if (notification.notificationType == 0) {
