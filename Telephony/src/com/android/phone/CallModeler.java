...
//import com.android.internal.telephony.ConferenceCallXml;  // mark volte part
...
import com.mediatek.phone.volte.VoLteConfModeler;
...
public class CallModeler extends Handler {
    ...
    private boolean updateForConferenceCalls(Connection connection, List<Call> updatedCalls) {
        ...
        boolean isConferenceCallConnection = isPartOfLiveConferenceCall(connection) &&
                getEarliestLiveConnection(connection.getCall()) == connection;
        /// M: for VoLte Conference Call @{
        if(FeatureOption.isMtkVolteSupport() && VoLteConfUtils.isVoLteConfCall(connection)) {
            isConferenceCallConnection = VoLteConfUtils.isVoLteConfMainConnection(connection)
                    && connection.isAlive();
        }
        /// @}
    ...
    private boolean updateCallFromConnection(Call call, Connection connection,
        ...
        if (oldConnectTime != connection.getConnectTime()) {
            ...
            // M: Add for ALPS01529355:The call timer will be affected when the system time changed.
            ...
            call.setConnectTimeReal(SystemClock.elapsedRealtime() - connection.getDurationMillis());
            changed = true;
        }
    ...
    private boolean handleMTKMessage(Message msg) {
        switch (msg.what) {
            ...
            /// M: for VoLTE Conference Call @{
                /* // mark volte part
            case CallStateMonitor.VOLTE_CONFERENCE_CALL_INDICATION:
                onVoLteConferenceCallIndication((AsyncResult) msg.obj);
                break;
                */
            /// @}
            ...
            case CallStateMonitor.PHONE_HANDLE_DISCONNECTING:
                onHandleForceRelease(msg.obj);
                break;
            /// @}
            default:
                return false;
    ...
    private void updateCallFromConnectionMTK(Call call, Connection connection) {
        ...
        /* // mark volte part
        if(FeatureOption.isMtkVolteSupport() && connection instanceof GsmConnection) {
            String pau = ((GsmConnection)connection).getPau();
            Log.d(TAG, "update pau field,  pau = " + pau);
            call.setPAUField(pau);
        }
        */
        /// @}
        ...
        /* // mark volte part
        if(FeatureOption.isMtkVolteSupport() && connection instanceof GsmConnection) {
            int callMode = VoLteConfUtils.getIMSCallMode(connection);
            call.setIMSCallMode(callMode);
            Log.d(TAG, "setIMSCallMode,  callmode = " + callMode);
            if(VoLteConfUtils.isVoLteConfCall(connection)) {
                int conferenceId =  VoLteConfUtils.getIMSConferenceId(connection);
                call.setConferenceId(conferenceId);
                Log.d(TAG, "setConferenceId,  conferenceId = " + conferenceId);
            }
        }
        */
        /// @}
    ...
    public void markDialingCallAsECC() {
        Log.d(TAG, "markDialingCallAsECC~~");
        com.android.internal.telephony.Call telephonyCall = mCallManager.getActiveFgCall();
        if (telephonyCall != null
                && (com.android.internal.telephony.Call.State.DIALING == telephonyCall.getState() || com.android.internal.telephony.Call.State.ALERTING == telephonyCall
                        .getState())) {
            Connection connection = telephonyCall.getLatestConnection();
            final Call call = getCallFromMap(mCallMap, connection, true);
            Log.d(TAG, "get dialing call, call = " + call);
            if(call != null) {
                // mark the MO call as ECC
                call.markAsECCCall();
            }
        }
    }
    ...
    public void markDialingCalltoWaiting() {
        Log.d(TAG, "markDialingCalltoWaiting~~");
        com.android.internal.telephony.Call telephonyCall = mCallManager.getActiveFgCall();
        if (telephonyCall != null
                && (com.android.internal.telephony.Call.State.DIALING == telephonyCall.getState() || com.android.internal.telephony.Call.State.ALERTING == telephonyCall
                        .getState())) {
    ...
    /// M: for VoLTE Conference Call @{
    public Call getVoLteConfedCall(Connection connection) {
        Call call =  getCallFromMap(mCallMap, connection, true);
        return call;
    }

    void onVoLteConferenceCallIndication(AsyncResult asyncResult) {
        /*// mark volte part
        ConferenceCallXml xmlData = (ConferenceCallXml) asyncResult.result;
        VoLteConfModeler.getInstance().processConferenceCallIndication(xmlData);
        */
    }
    /// @}
    ...
    /// M: Change Feature  ALPS00811599 @{
    // Call not hangup when network is abnormal
    public void requestDelayDisconnecting(Object obj) {
        Log.d(TAG, "requestDelayDisconnecting()...");
        removeMessages(CallStateMonitor.PHONE_HANDLE_DISCONNECTING);

        Message msg = obtainMessage(CallStateMonitor.PHONE_HANDLE_DISCONNECTING, obj);
        sendMessageDelayed(msg, CallStateMonitor.DISCONNECTING_INFO_TIMEOUT);

    }

    private void onHandleForceRelease(Object obj) {
        Log.d(TAG, "PHONE_HANDLE_DISCONNECTING ! ");
        PhoneUtils.handlePendingCall(obj);
    }
    /// @}
