...
//        }
    ...
    private void showNetworkSelection(String operator, int slotId) {
         ...
         * M: plugin @{
         */
        ExtensionManager.getPhoneMiscExt().customizeNetworkSelectionNotification(notification, titleText, expandedText, pi);
        /** @} */
    ...
    private int[][] mCfiIconMapDual = {
            {R.drawable.mtk_stat_sys_phone_call_forward_blue_sim1, R.drawable.mtk_stat_sys_phone_call_forward_blue_sim2},
            {R.drawable.mtk_stat_sys_phone_call_forward_orange_sim1, R.drawable.mtk_stat_sys_phone_call_forward_orange_sim2},
            {R.drawable.mtk_stat_sys_phone_call_forward_green_sim1, R.drawable.mtk_stat_sys_phone_call_forward_green_sim2},
            {R.drawable.mtk_stat_sys_phone_call_forward_purple_sim1, R.drawable.mtk_stat_sys_phone_call_forward_purple_sim2},
            {R.drawable.stat_sys_phone_call_forward_sim1, R.drawable.stat_sys_phone_call_forward_sim2},
    };
    ...
    private void updateCfiForAllSlots() {
        for (int index = GeminiUtils.getSlots().length - 1; index >= 0; index--) {
            ...
            if (mCfiStatusMap.get(slotId)) {
                ...
                if (SystemProperties.getBoolean("ro.tcl.cu", false)) {
                	//fix bug 8747:被测终端设置卡1（卡2）始终呼叫转移后，在状态栏会出现来电转接的标识，但未指明哪张卡开启的来电转接功能。
                	if (simInfo != null && simInfo.mColor >= 0 && simInfo.mColor < mCfiIconMapDual.length) {
                        resId = mCfiIconMapDual[simInfo.mColor][slotId];
                    } else {
                    	resId = mCfiIconMapDual[mCfiIconMapDual.length - 1][slotId];
                    }
                } else {
