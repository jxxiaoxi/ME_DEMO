...
public class CallHandlerServiceProxy extends Handler
 implements CallModeler.Listener, AudioModeListener,
        ...
        VTManagerWrapper.Listener, VoLteConfModeler.Listener,
        CallCommandService.Listener {
    ...
    public CallHandlerServiceProxy(Context context, CallModeler callModeler,
        ...
        mVoLteConfModeler = VoLteConfModeler.getInstance();
        mVoLteConfModeler.addListener(this);
        /// @}
    }
    ...
    private void processUpdate(List<Call> calls) {
        ...
        try {
            ...
            ///M: when there are some important MSG in Queue, should not unbind immediately.
    ...
    private static class QueueParams {
        ...
        /// M: For VT @{
        private static final int METHOD_VT_SETTING_PARAMS = 4;
        private static final int METHOD_VT_STATE_CHANGE = 5;
...
//-------------------MTK----------------------------
    ...
    /// M: for VoLTE Conference Call @{
    private VoLteConfModeler mVoLteConfModeler;
        ...
        try {
            synchronized (mServiceAndQueueLock) {
                if (mCallHandlerServiceGuarded == null) {
                    if (DBG) {
                        Log.d(TAG, "CallHandlerService not conneccted. Skipping "
                                + "onVoLteConferenceUpdate.");
                    }
                    return;
                }
            }
            Log.d(TAG, "onVoLteConferenceUpdate, conferenceId =" + conferenceId);
            mCallHandlerServiceGuarded.onVoLteConferenceUpdate(conferenceId, members);
        } catch (Exception e) {
            Log.e(TAG, "Remote exception handling onVoLteConferenceUpdate()", e);
        }
    }
    /// @}
}
