...
/// @}
    ...
    public void onCreate() {
        ...
        ///M: DAUL TALK SUPPORT @{ 
        if (FeatureOption.isMtkDtSupport()) {
        ...
        public void handleMessage(Message msg) {
            ...
            switch(msg.what) {
                ...
                case PHONE_ON_DISCONNECT:
                    ...
                    /// M: Delay SCO Disconnect for Busy Tone
                    if(msg.what == PHONE_ON_DISCONNECT && connection != null){    
    ...
    private long getCdmaHoldTime(Call c){
        ...
        if(c.getEarliestConnection()!= null){
            cdmaHoldTime = c.getEarliestConnection().getCreateTime() ;
        }
    ...
    private Call chooseTheEarlierHoldCall(Call c1, Call c2){
        ...
        if(c1.getEarliestConnection() != null){
            c1Time = c1.getEarliestConnection().getCreateTime();
        }
        
        if(c2.getEarliestConnection() != null){
            c2Time = c2.getEarliestConnection().getCreateTime();
        }
    ...
    Call chooseTheLatterCall(Call c1, Call c2){
        long c1_createTime = 0;
        long c2_createTime = 0;
        
        if(c1.getEarliestConnection() != null){
            c1_createTime = c1.getEarliestConnection().getCreateTime();
        }

        if(c2.getEarliestConnection() != null){
            c2_createTime = c2.getEarliestConnection().getCreateTime();
        }
        
        return (c1_createTime > c2_createTime) ? c1 : c2;
    }
    ...
    private long generateCdmaHoldTime(Call activeForegroundCall){
        ...
        if(activeForegroundCall.getEarliestConnection() != null){
            maxHoldTime = activeForegroundCall.getEarliestConnection().getCreateTime();
        }

        List<Call> listCall = mCM.getBackgroundCalls();
        for(Call c : listCall){
            if(Call.State.HOLDING == c.getState()){
                Connection connection = c.getEarliestConnection();
                if(connection != null && maxHoldTime < connection.getCreateTime()){
    ...
    private void updateCdmaHoldTime(boolean bCanSwapInDiffSim){
        ...
        if(bCanSwapInDiffSim){
            if(null != mHashCdmaHoldTime){
                ...
                if(foregroundCall != null && PhoneConstants.PHONE_TYPE_GSM == foregroundCall.getPhone().getPhoneType() && foregroundCall.getState().isAlive()){
    ...
    private boolean executeHHRestrictIfNeed(boolean bIsConnected){
        ...
        if(activeFgCall != null && (Call.State.ACTIVE == activeFgCall.getState() || activeFgCall.getState().isDialing()) ){
    ...
    private boolean checkUnreasonableStates(){
        ...
        if( true != FeatureOption.isEvdoDtSupport() ){
            for(Call c1 : fgCalls){
...
/// @}
    ...
    private void handlePreciseCallStateChangeDualTalk(Connection connection) {
        ...
        if(mForegroundCallState == Call.State.ACTIVE){
            mNumActive = 1;
            if (foregroundCall != null && foregroundCall.getPhone() != null) {
            ...
            if(activePhoneType == PhoneConstants.PHONE_TYPE_GSM){
                if (mPhoneGlobals.cdmaPhoneCallState != null){
                    ...
                    if(currCdmaCallState != CdmaPhoneCallState.PhoneCallState.IDLE){
                        ...
                        if(CdmaFgCall != null && CdmaFgCall.getState() != Call.State.DISCONNECTING && CdmaFgCall.getState() != Call.State.DIALING && CdmaFgCall.getState() != Call.State.ALERTING)
                            mNumHeld = 1 ;
                    }
                }
    ...
    private void sendClccResponseEVDO(int index, Connection c) {
        ...
        if ((phoneType == PhoneConstants.PHONE_TYPE_CDMA) && (prevCdmaCallState == CdmaPhoneCallState.PhoneCallState.THRWAY_ACTIVE) && (currCdmaCallState == CdmaPhoneCallState.PhoneCallState.CONF_CALL)){
            ...
            switch(c.getState()){
                    ...
                    if(phoneType == PhoneConstants.PHONE_TYPE_CDMA){
                        ...
                        for(Call fCall : fCalls){
                            ...
                            if(phoneType == PhoneConstants.PHONE_TYPE_CDMA){
                                ...
                                // if the internal getActiveFgCall returns GSM call, it means GSM call is not idle, thus consider CDMA FG call as Hold 
                                Call fgCall = mBtDTUtil.getActiveFgCall();
                                int cPhoneType = fgCall.getPhone().getPhoneType();
                                if((PhoneConstants.PHONE_TYPE_GSM == cPhoneType))
                                {
                                    state = 1;
                                    found = true;
                                    log("evdoConnectionToClccEntry : CDMA as hold call for GSM non-idle foreground call existed");
                                    break;
                                }
                ...
                case INCOMING:
                {
                    if ( (backgroundCall != null && backgroundCall.getState().isAlive()) || (foregroundCall != null && foregroundCall.getState().isAlive()) ) {
...
///@}
    ...
    private DT_EVDO_SCENARIO evdoAnalyzeDTScenario() {
        ...
        if(FeatureOption.isEvdoDtSupport() == false)
        {
    ...
    private boolean evdoDTChldHandler0(DT_EVDO_SCENARIO scenario) {
        ...
        switch(scenario){
            ...
            case DT_SCENARIO_IAH_N:{
                ...
                try{
                    if(ringingCall != null){
                        ...
                        log("CHLD 0 : ringingCall is null");
                        return false;
                    }
            ...
            case DT_SCENARIO_H_IA:
            {
                ...
                try{
                    if(ringingCall != null){
                        Connection latestRingCallConnection = ringingCall.getLatestConnection();
                        if(latestRingCallConnection != null){
                            latestRingCallConnection.hangup();
                        }
                    }else{
                        log("CHLD 0 : ringingCall is null");
                    }
            ...
            case DT_SCENARIO_AH_N:{
                ...
                try{
                    if(backgroundCall != null){
                        ...
                        log("CHLD 0 : background call is null");
                    }
                } catch (CallStateException ex){
                    log("CHLD 0 : catch CallStateException");
                    return false;
            ...
            case DT_SCENARIO_H_IA:
            {
                ...
                try{
                    if(backgroundCall != null){
                        ...
                        log("CHLD 1 : backgroundCall is null");
                    }
                } catch (CallStateException ex){
                    log("CHLD 1 : catch CallStateException");
                    return false;
    ...
    private boolean evdoDTChldHandler2(DT_EVDO_SCENARIO scenario) {
        ...
        switch(scenario){
            ...
            case DT_SCENARIO_A_N:
            {
                ...
                try{
                    if(foregroundCall != null){
                        if(foregroundCall.getPhone() != null){
                            foregroundCall.getPhone().switchHoldingAndActive();
                        }else{
                            log("CHLD 2 : foregroundCall.getPhone() is null");
                        }
                    }else{
                        log("CHLD 2 : foregroundCall is null");
                    }
                } catch (CallStateException ex){
            ...
            case DT_SCENARIO_A_H:
            {
                for(Call aCall : fCalls){
                    if(aCall.getState() == Call.State.ACTIVE){
                        phoneType = aCall.getPhone().getPhoneType();
                        if(phoneType == PhoneConstants.PHONE_TYPE_CDMA){
                            log("CHLD 2 : switch Holding and Active with CDMA active call");
                            PhoneUtils.switchHoldingAndActive(aCall);
                            return true;
                        }
                    }
                }
            ...
            if (phoneType == PhoneConstants.PHONE_TYPE_CDMA) {
                ...
                if (mCM.hasActiveFgCall() && mCM.hasActiveBgCall()) {
                    ...
                    if (FeatureOption.isMtkDtSupport()) {
                        ...
                        if(mCM.getActiveFgCall() != null){
                            fgPhone = mCM.getActiveFgCall().getPhone();
                        }

                        if(mCM.getFirstActiveBgCall() != null){
                            bgPhone = mCM.getFirstActiveBgCall().getPhone();
                        }
...
///}@
    ...
    private final IBluetoothHeadsetPhone.Stub mBinder = new IBluetoothHeadsetPhone.Stub() {
        ...
        ///M: DAUL TALK SUPPORT  @{ 
            if(FeatureOption.isEvdoDtSupport() == true){
        ...
        public String getNetworkOperator() {
            ...
            Phone phone = CallManagerWrapper.getDefaultPhone(slotId);
            if(phone != null)
            {
                return phone.getServiceState().getOperatorAlphaLong();
            }else{
                return null;
            }
            /// @}
    ...
    private void registerPhoneEvents(boolean register){
        if(VDBG) Log.d(TAG, "registerPhoneEvents:" + register + " Gemini:" + FeatureOption.isMtkGeminiSupport());
        if (register) {
