    ...
    public class OutgoingCallReceiver extends BroadcastReceiver {
        ...
        public boolean doReceive(Context context, Intent intent) {
             ...
             */
                ...
                /// M: For ALPS01527880. Imagine the case which IP prefix is "11" and phone number is "2",
                 // and making a call from CallDetailsActivity's IP call menu. The number
                 // is "2" at processIntent() and has been changed to "112" after going through
                 // IpCallOptionHandler. This call shall be unexpectly given up here by this
                 // Emergency checking just for 3rd-party apps. Thus we need to especially let
                 // this kind of IP call go forward.
                if (isPotentialLocalEmergencyNumber && !intent.getBooleanExtra(
                        Constants.EXTRA_IS_IP_DIAL, false)) {
                /* }@ */
            ...
            /// M: For VT drop back @{
            // if current network do not support VT, and is not auto drop. just finish this activity.
            // Or if user click "confirm" on Drop back dialog quickly before
            // this is finished, AMS will move old task foreground, see ALPS01529056.
            boolean isVTRequest = intent.getBooleanExtra(Constants.EXTRA_IS_VIDEO_CALL, false);
            int slotId = intent.getIntExtra(Constants.EXTRA_SLOT_ID, GeminiUtils.getDefaultSlot());
            if (isVTRequest && !VTCallUtils.isUnder3GService(slotId) && !VTCallUtils.isAutoDropBack(slotId)) {
                return false;
            }
            /// @}
            return true;
