...
public class PhoneGlobals extends ContextWrapper implements WiredHeadsetListener {
     ...
     */
    /* package */ static final int DBG_LEVEL = 2; /// M: update level to 2, original 0
...
//            }
            ...
            */
            ...
            intentFilter.addAction("phone_go_to_sleep");
            if (mTtyEnabled) {
    ...
    */
    ...
    private class PhoneGlobalsBroadcastReceiver extends BroadcastReceiver {
        ...
        public void onReceive(Context context, Intent intent) {
			...
			// add by liuwei for 5s srceen off
			boolean isPhoneLock = SystemProperties.getBoolean("ro.phone.lock",
					false);
			Log.e("huiye", " PhoneGlobals select : " + isPhoneLock
					+ " ;action : " + action);
			if (isPhoneLock && action.equals("phone_go_to_sleep")) {
    ...
    private void initForMtkFeatures() {
        ExtensionManager.getPhoneMiscExt().onPhoneGlobalsCreate(this, phone);
    ...
    private IntentFilter addActionForGlobalReceiver(IntentFilter intentFilter) {
        ...
        intentFilter.addAction(PhoneWrapper.EVENT_CAPABILITY_SWITCH_START_MD_RESET);
    ...
    private boolean onReceiveGlobalMtkActions(Context context, Intent intent) {
        ...
        if (action.equals(Intent.ACTION_DUAL_SIM_MODE_CHANGED)) {
            ...
            // If ipo is open, com.android.phone will not be killed.
            // we just hang up calls and will receive DISCONNECT message later, which can clear CallModeler and add callLog.
            // Or com.android.phone will be killed and will not receive ACTION_SHUTDOWN_IPO intent.
            // under this condition, if com.android.phone was killed before receiving DISCONNECT message,
            // will lose callLog for current calls, so we addCurrentCalltoCallLogs().
            int ipoStatus = Settings.System.getInt(getApplicationContext().getContentResolver(),
                    Settings.System.IPO_SETTING, 0);
            PhoneLog.d(LOG_TAG, "ipoStatus = " + ipoStatus);
            if (1 == ipoStatus) {
            ...
            PhoneLog.d(LOG_TAG, "onReceiveMtkActions:::NETWORK_MODE_CHANGE, gemini set mode");
            ...
            if (PhoneFactory.isDualTalkMode()) {
                SystemProperties.set("gsm.ril.eboot.2", "0");
            }
            /// @}
            ...
            /// @}
        } else if (action.equals(PhoneWrapper.EVENT_CAPABILITY_SWITCH_START_MD_RESET)) {
            Settings.System.putLong(getApplicationContext().getContentResolver(),
                    Settings.System.SIM_LOCK_STATE_SETTING, 0x0L);
    ...
    private void onReceiveAirplanModeChange(Context context, Intent intent) {
        ...
        if (GeminiUtils.isGeminiSupport()) {
            GeminiRadioStatusUtil.mAirplaneMode = enabled ? 1 : 0;
            PhoneLog.d(LOG_TAG, "setRadioStatusUtil's airplane" + GeminiRadioStatusUtil.mAirplaneMode);
        }
        ...
        if (FeatureOption.isMtkFlightModePowerOffMd()) {
            ...
            if (enabled) {
                ...
                if (enabled) {
                    ...
                    if (PhoneFactory.isDualTalkMode()) {
                        SystemProperties.set("gsm.ril.eboot.2", "1");
                    }
    ...
    private boolean handleMessageMtk(Message msg) {
        ...
        switch (msg.what) {
            ...
            case MESSAGE_SET_PREFERRED_NETWORK_TYPE:
                ...
                PhoneLog.d(LOG_TAG, "handleMessageMtk, :::NETWORK_MODE_CHANGE_RESPONSE exception =" + ar.exception);
                if (ar.exception == null) {
                    it.putExtra(NETWORK_MODE_CHANGE_RESPONSE, true);
                    it.putExtra("NEW_NETWORK_MODE", msg.arg2);
                    PhoneLog.d(LOG_TAG, "handleMessageMtk, ::: NEW_NETWORK_MODE =" + msg.arg2);
                } else {
