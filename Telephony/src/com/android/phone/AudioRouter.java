    ...
    */
    ...
    public void setAudioMode(int mode) {
        ...
        if (AudioMode.SPEAKER == mode) {
            /// M: for ALPS01524937 @{
            ///when turn on speaker not finished, don't notify other mode to IncallUI.
            //Original code:
            //turnOnOffBluetooth(OFF);
            //turnOnOffSpeaker(ON);
            mChangeBT2SpeakerFinished = false;
            turnOnOffBluetooth(OFF);
            turnOnOffSpeaker(ON);
            mChangeBT2SpeakerFinished = true;
            /// @}
    ...
    public void onMuteChange(boolean muted) {
        ...
        ///M: For ALPS01509993.
        // This will notify the incallui's audio mode sonner.
        // Before notify, we should calculate the status again. @{
        logD("onMuteChange: Before mAudioMode = " + mAudioMode + "; "
                + "mSupportedModes = " + mSupportedModes);
        if (calculateModeFromCurrentState() != mAudioMode) {
            mAudioMode = calculateModeFromCurrentState();
        }
        if (calculateSupportedModes() != mSupportedModes) {
            mSupportedModes = calculateSupportedModes();
        }
        logD("onMuteChange: After mAudioMode = " + mAudioMode + "; "
                + "mSupportedModes = " + mSupportedModes);
        /// @}
        notifyListeners();
    }
    ...
    public void onBluetoothIndicationChange(boolean isConnected, BluetoothManager btManager) {
        ...
        if (!mChangeBT2SpeakerFinished){
            logD("onBluetoothIndicationChange: still in seAudioMode flow, return");
            return;
        }
    ...
    private int calculateModeFromCurrentState() {
        ...
        // For ALPS01509993 Modify the order of audio mode caculation.
        // 1. If the voice is from speaker, change mode to SPEAKER
        // 2. If show bluetooth indication is true, change mode to BLUETOOTH
        // 3. If the headset is plugged in, change mode to WIRED_HEADSET.@{
        if (PhoneUtils.isSpeakerOn(mContext)) {
    ...
    //---------------------------- MTK --------------------------------
    private boolean mChangeBT2SpeakerFinished = true;
}
