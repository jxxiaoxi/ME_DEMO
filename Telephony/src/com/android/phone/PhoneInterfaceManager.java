...
public class PhoneInterfaceManager extends ITelephony.Stub implements CallModeler.Listener {
    ...
    public void call(String callingPackage, String number) {
        ...
        if (MobileManagerUtils.isMomSupport()) {
    ...
    private void answerRingingCallInternal() {
        ...
        com.android.internal.telephony.Call ringing = mCM.getFirstActiveRingingCall();
    ...
    public int getNetworkType() {
        ...
        if (mPhone.getServiceState().getDataNetworkType() != ServiceState.RIL_RADIO_TECHNOLOGY_UNKNOWN){
            return mPhone.getServiceState().getDataNetworkType();
        } else {
            // return voice network type for better backward compatible
            return mPhone.getServiceState().getVoiceNetworkType();
        }
        //[ALPS01620689]-ADD : End
    }
    ...
    /**
     * only add this api since PhoneInterfaceManager 
     * implements CallModeler.Listener, to avoide build error.
     * @hide
     */
    public void onSuppServiceFailed(String errorMessage){}
    /**
     * only add this api since PhoneInterfaceManager 
     * implements CallModeler.Listener, to avoide build error.
     * @hide
  ...
  public String simAuth(String strRand) {
    ...
    */
    ...
    /** {@hide} */
    public int[] supplyPinReportResultEx(String pin, int simId) {
        IccCard iccCard = null;
        enforceModifyPermission();
        if (GeminiUtils.isGeminiSupport()) {
            iccCard = ((GeminiPhone)mPhone).getPhonebyId(simId).getIccCard();
        } else {
            iccCard = mPhone.getIccCard();
        }
        final UnlockSim checkSimPin = new UnlockSim(iccCard);
        checkSimPin.start();
        return checkSimPin.unlockSim(null, pin);
    }

    /** {@hide} */
    public int[] supplyPukReportResultEx(String puk, String pin, int simId) {
        IccCard iccCard = null;
        enforceModifyPermission();
        if (GeminiUtils.isGeminiSupport()) {
            iccCard = ((GeminiPhone)mPhone).getPhonebyId(simId).getIccCard();
        ...
        final UnlockSim checkSimPuk = new UnlockSim(iccCard);
        checkSimPuk.start();
        return checkSimPuk.unlockSim(puk, pin);
    }
    ...
    public IPhoneSubInfo getSubscriberInfo(int simId) {
		...
		if(simId < 0){
			Log.d( LOG_TAG,"java.lang.ArrayIndexOutOfBoundsException simId =" + simId);
			return null;
		}
...
*/
    ...
    */
    ...
    /**
      Description : Adjust modem radio power for Lenovo SAR requirement.
      AT command format: AT+ERFTX=<op>,<para1>,<para2>,<para3>
      Description : When <op>=1	 -->  TX power reduction
				    <para1>:  2G L1 reduction level, default is 0
				    <para2>:  3G L1 reduction level, default is 0
				    <para3>:  4G L1 reduction level, default is 0
				    level scope : 0 ~ 64
      Arthur      : mtk04070
      Date        : 2014.03.05
      Return value: True for success, false for failure
    */
    public boolean adjustModemRadioPower_234G(int level_2G, int level_3G, int level_4G) {
        boolean result = ((level_2G >= 0) && (level_2G <= 255) && 
                          (level_3G >= 0) && (level_3G <= 255) &&
                          (level_4G >= 0) && (level_4G <= 255));
        Log.d(LOG_TAG, "adjustModemRadioPower for 2G/3G/4G");
        if (true == result) {
            String cmdStr[] = { "AT+ERFTX=1,", "" };
            cmdStr[0] = cmdStr[0] + level_2G + "," + level_3G + "," + level_4G;
            mPhone.invokeOemRilRequestStrings(cmdStr, 
		   	                mMainThreadHandler.obtainMessage(CMD_ADJUST_MODEM_RADIO_POWER));
            Log.d(LOG_TAG, cmdStr[0] + " ");
        }
    ...
    */
    public boolean adjustModemRadioPowerByBand(int rat, int band, int level) {
        ...
        int i, headParameters, restParameters; 
  ...
  /**
     ...
     * In multiple SIM project, this API will return located PLMN as possible. (ex: When SIM1 is turned off , then we return located PLMN information from SIM2 modem protocol)
     * @hide
     * @internal 
     */
