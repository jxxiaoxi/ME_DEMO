...
public class CallController extends Handler {
    ...
    private CallStatusCode placeCallInternal(Intent intent) {
        ...
        log("placeCallInternal() FeatureOption.isMtkVT3G324MSupport() = " + FeatureOption.isMtkVT3G324MSupport());

        /// M: for VoLTE IMS call @{
        boolean isIMSOnly = false;
        if (FeatureOption.isMtkVolteSupport()) {
            isIMSOnly = intent.getBooleanExtra(Constants.EXTRA_IS_IMS_CALL, false);
            log("isIMSOnly = " + isIMSOnly);
        }
        /// @}
        ...
        /// M: For ALPS01868286 @{
        // abandon the dial request while Ecc exist.
        if (hasEcc()) {
            log("has Ecc already, mark this dial request as <Call not sent.>");
        ...
        if (isEmergencyNumber) {
            ...
            /// M: for VoLTE @{
            if(isIMSOnly) {
                intent.putExtra(Constants.EXTRA_IS_IMS_CALL, false);
                isIMSOnly = false;
            }
            /// @}
        }
        ...
        */
        ...
        /// for VoLTE IMS Call @{
        // If IMS registration status is true, modify okToCallStatus to make this call have chance to be placed through IMS.
        /* // mark volte part
        if (FeatureOption.isMtkVolteSupport() && okToCallStatus != CallStatusCode.SUCCESS) {
            if (phone.getImsRegInfo()) {
                if (DBG) log("==> IMS registration status is true, UPDATING status to SUCCESS from :" + okToCallStatus);
                okToCallStatus = CallStatusCode.SUCCESS;
            }
        }
        */
        ...
        if (FeatureOption.isMtkVT3G324MSupport() && isVideoCall) {
    ...
    private boolean placeCallPreCheck(Intent intent, String number) {
        ...
        /// M: for ALPS01868286 @{
        // Ecc exist and Ecc request, we should keep the former one and abandon the new Ecc request.
        // We will handle this new Ecc request as "Call not sent" later in placeCallInternal().
        if (isEccCall && hasEcc()) {
            return true;
        }
        /// @}
    ...
    private int placeCallInternalPlaceCall(Phone phone, String number, Uri contactUri,
            boolean isEmergencyCall, RawGatewayInfo rawGatewayInfo, CallGatewayManager callGatewayManager, int slot, boolean isVideoCall, boolean isIMSOnly) {
        ...
        if (FeatureOption.isMtkVT3G324MSupport() && isVideoCall) {
            ...
            return PhoneUtils.placeCallGemini(mApp, phone, number, contactUri, isEmergencyCall,
                    rawGatewayInfo, callGatewayManager, slot, isIMSOnly);
        }
        return PhoneUtils.placeCall(mApp, phone, number, contactUri, isEmergencyCall, rawGatewayInfo, callGatewayManager, isIMSOnly);
    }
    ...
    /**
     * Check whether has Ecc already.
     * @return
     */
    private boolean hasEcc() {
        boolean result = false;
        List<Connection> connections = new ArrayList<Connection>();
        connections.addAll(mCM.getFgCallConnections());
        connections.addAll(mCM.getBgCallConnections());
        for (Connection connection : connections) {
            if (!TextUtils.isEmpty(connection.getAddress()) &&
