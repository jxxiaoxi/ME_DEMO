...
public class CallTime extends Handler {
    ...
    private static long getCallDurationMTK(Call call) {
        if (FeatureOption.isMtkVT3G324MSupport() && null != call.getLatestConnection()
                && call.getLatestConnection().isVideo()) {
            if (call.getLatestConnection() != VTCallFlags.getInstance().mVTConnectionStarttime.mConnection) {
                return 0;
            }
            if (VTCallUtils.VTTimingMode.VT_TIMING_NONE == VTCallUtils.checkVTTimingMode(call
                ...
                if (VTCallFlags.getInstance().mVTConnectionStarttime.mStarttime < 0) {
                    ...
                    return SystemClock.elapsedRealtime() - VTCallFlags.getInstance().mVTConnectionStarttime.mStarttime;
                }
            }
