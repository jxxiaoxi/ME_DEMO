...
public class CallNotifier extends Handler
    ...
    private CallNotifier(PhoneGlobals app, Phone phone, Ringer ringer, CallLogger callLogger,
        ...
        mAudioManager = (AudioManager) mApplication.getSystemService(Context.AUDIO_SERVICE);
     	...
     	mAm = (ActivityManager)mApplication.getSystemService(Context.ACTIVITY_SERVICE);
        ...
        switch (msg.what) {
            ...
            case CallStateMonitor.PHONE_INCOMING_RING:
                ...
                if (msg.obj != null && ((AsyncResult) msg.obj).result != null) {
                    /// M: For should send to voice mail and other should ignored incoming call. @{
                    // Original Code:
                    ...
                    //add++ 
                        if(bRingerDelay){ 
                    PendingRingMessage = Message.obtain(this, CallStateMonitor.PHONE_INCOMING_RING, msg.obj); 
                         log("do PendingRingMessage !"); 
                         }else{ 
                    //add-- 
                        mRinger.ring();
                    //add++ 
...
//            }
    ...
    private void ringAndNotifyOfIncomingCall(Connection c) {
        ...
        /// M: For Dualtalk @{
        // dismiss callSelectDialog when Incoming call UI is shown, see ALPS01619623.
        // TODO: maybe we should move callSelectDialog to InCallUI to manage.
        if (DualTalkUtils.isSupportDualTalk()) {
            PhoneUtils.dismissCallSelectDialog();
        }
        /// @}
    }
    ...
    private void onPhoneStateChanged(AsyncResult r) {
        /// M: For Plug-in @{
        ExtensionManager.getRCSeCallNotifierExt().onPhoneStateChanged(mCM, mApplication.getApplicationContext());
        /// @}
            ...
            */
            ...
            if (isQueryExecutionTimeOK) {
                ...
                /// M: For DualTalk @{
                    ...
                    // so here record its default ringtone if not contactRingtoneUri presented.
                    Uri uri = getDefaultRingtoneUri(c.getCall());
                    ci.contactRingtoneUri = uri;
                    log("set call's uri = " + uri + " for " + ci);
                }
    ...
    /// @}
        ...
        if (c != null) {
            ...
            // when disconnected by network with ECC reregister reason
            // we should redial, then framework will handle this redial as ECC dial
            /* // mark volte part
            if(FeatureOption.isMtkVolteSupport() && cause == Connection.DisconnectCause.IMS_EMERGENCY_REREG) {
                redialAsECC(number, phone);
            }
            */
            /// @}
        }
        ...
        // Add for plugin.
        ExtensionManager.getRCSeCallNotifierExt().onDisconnect(c);
    }
    ...
    private class InCallTonePlayer extends Thread {
        ...
        public void run() {
            ...
            switch (mToneId) {
                ...
                case TONE_BUSY:
                    ...
                    if (FeatureOption.isMtkBrazilCustomizationVivo()) {
        ...
        private void setAudioParametersForEVDO() {
            if (FeatureOption.isEvdoDtSupport()) { // only for CDMA
    ...
    /// @}
        ...
        if (info != null) {
            ...
            if (info.isFinal) {
                ...
                if(!isRunning("com.jrdcom.care.launcher"))
...
if(SystemProperties.getBoolean("ro.tcl_zhiyi", false))
{

    List<RunningAppProcessInfo> infos = mAm.getRunningAppProcesses();
    ...
    private void initCallNotifierMtk() {
        ...
        if (FeatureOption.isMtkMultisimRingtoneSupport()) {
    ...
    private void clearVTRelatedIfNeed(Connection c, int simId) {
        if (FeatureOption.isMtkVT3G324MSupport()) {
            if (c.isVideo()) {
                if (VTCallFlags.getInstance().mVTShouldCloseVTManager
                // Remember when single sim card, slot (sim) id is -1 !!!!!!!!
                    ...
                    if (!VTCallUtils.isVTActive()) {
                        ...
                        if (FeatureOption.isMtkPhoneVoiceRecordingSupport()) {
                    ...
                    if (DBG)
                        log("onDisconnect: set VTCallFlags.getInstance().mVTShouldCloseVTManager = true");
                    VTCallFlags.getInstance().mVTShouldCloseVTManager = true;
                }
            }
        }
    }
    ...
    /// M: For New feature: MO vibrate when call get connected. @{
    // bit1(0000X0) in Settings.Global.TELEPHONY_MISC_FEATURE_CONFIG indicate the on-off key (1-on, 0-off).
    // this can be set in EM.
    private static final int KEY_MO_VIBRATE_CONFIG = 0x00000002;
    ...
    private void vibrateWhenMOConnected() {
        Call.State newCallState = mCM.getActiveFgCallState();
        if (DBG) {
            Log.d(LOG_TAG, "vibrateWhenMOConnected()...mPreviousCallState / newCallState: "
                    + mPreviousCallState + " => " + newCallState);
        }

        if ((newCallState == Call.State.ACTIVE) && (mPreviousCallState == Call.State.ALERTING)) {
            int emSetting = Settings.Global.getInt(mApplication.getContentResolver(),
                    Settings.Global.TELEPHONY_MISC_FEATURE_CONFIG, KEY_MO_VIBRATE_CONFIG);
            boolean enable = (emSetting & KEY_MO_VIBRATE_CONFIG) != 0;
            Log.d(LOG_TAG, "vibrateWhenMOConnected()...  emSetting =" + emSetting + " enable =" + enable);
            if (enable) {
    ...
    void switchRingToneByNeeded(Call ring) {
        if (PhoneUtils.isRealIncomingCall(ring.getState())) {
            ...
            if (custUri != null && !custUri.equals(mRinger.getCustomRingToneUri())) {
                mRinger.stopRing();
    ...
    private boolean handleMessageMTK(Message msg) {
        ...
        case CallStateMonitor.PHONE_VT_RING_INFO:
            ...
            if (VTCallUtils.isVTRinging() && !mSilentRingerRequested && !mShouldSkipRing) {
        ...
        case EVENT_SIMINFO_CHANGED:
            for (int slot = 0; slot < mCfiStatus.length; slot++) {
                /// for ALPS01543152 when clear cfi icon, should set cfi status false.
                if (shouldNotifyCfiChange(slot)) {
                    onCfiChanged(true, slot);
                } else {
                    onCfiChanged(false, slot);
                    mCfiStatus[slot] = false;
                }
            }
            break;
    ...
    public Uri getDefaultRingtoneUri(Call ringCall) {
        ...
        if (ringCall == null) {
            log("getDefaultRingtoneUri: ringCall is null, just return " + customRingtoneUri);
        ...
        if (FeatureOption.isMtkGeminiSupport() && FeatureOption.isMtkMultisimRingtoneSupport() && mProfileMaqnager != null) {
            ...
            } else if (FeatureOption.isMtkVT3G324MSupport() && VTCallUtils.isVideoCall(ringCall)) {
            ...
            if (FeatureOption.isMtkVT3G324MSupport() && VTCallUtils.isVideoCall(ringCall)) {
    ...
    /**
     * ALPS01532025
     * in case that Call forwarding is set.
     * hotswap the icon should update.
     * when device is power on, turn on airplane mode, the cfu icon should display
     * howerver, when first airplane mode is open, and reboot cfu, (PLIGHT_MODE_POWER_OFF_MD  = true)
     * the icon is disappear.
     * @param slotId
     * @return if should return true, else false
     */
    private boolean shouldNotifyCfiChange(int slotId) {
        return mCfiStatus[slotId] && ITelephonyWrapper.hasIccCard(slotId);
    }
        ...
        switch (state) {
            case ACTIVE:
                ...
                if (VTCallUtils.isVideoCall(fgCall)
                        && VTCallFlags.getInstance().mVTConnectionStarttime.mStarttime < 0) {
...
//            return;
//        }
        if (VTCallFlags.getInstance().mVTConnectionStarttime.mStarttime < 0) {
            ...
            if (null != mCM.getActiveFgCall()) {
                if (mCM.getActiveFgCall().getLatestConnection() != null) {
                    VTCallFlags.getInstance().mVTConnectionStarttime.mStarttime = SystemClock
                            .elapsedRealtime();
                    VTCallFlags.getInstance().mVTConnectionStarttime.mConnection = mCM
    ...
    /// M: for VoLTE normal call switch to ECC @{
    private void redialAsECC(String number, final Phone phone) {
        log("redial as ECC~~");
        final int redialStatus =
            PhoneUtils.placeCall(mApplication, phone, number, null, false);
        if (redialStatus != PhoneUtils.CALL_STATUS_FAILED) {
            // mark it as ecc and update to UI
            CallModeler callModeler = PhoneGlobals.getInstance().getCallModeler();
            if (callModeler != null) {
                callModeler.markDialingCallAsECC();
                callModeler.updateCalls();
            }
        }
    }
    
    
  //Event used to delay play ring 
    private boolean bRingerDelay = false; 
    private Message PendingRingMessage = null; 
     
    void ringInCallActivityDone() 
    { 
     log("ringInCallActivityDone()"); 
     if(PendingRingMessage != null) 
     { 
      sendMessage(PendingRingMessage); 
      PendingRingMessage = null; 
     }else{ 
      log("PendingRingMessage = null !"); 
     } 
      bRingerDelay = false; 
     log("bRingerDelay = " + bRingerDelay); 
    }
