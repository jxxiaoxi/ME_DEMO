...
//Google: private static final boolean DBG = (PhoneGlobals.DBG_LEVEL >= 2);
    ...
    /* package */ static boolean answerAndEndActive(CallManager cm, Call ringing) {
        ...
        /// M: For VT @{
        if (FeatureOption.isMtkVT3G324MSupport() && VTCallUtils.isVTRinging()) {
    ...
    public static int placeCall(Context context, Phone phone, String number, Uri contactRef,
        ...
        return placeCall(context, phone, number, contactRef, isEmergencyCall,
                CallGatewayManager.EMPTY_INFO, null, false);
    }
     ...
     */
    public static int placeCall(Context context, Phone phone, String number, Uri contactRef,
            boolean isEmergencyCall, RawGatewayInfo gatewayInfo, CallGatewayManager callGateway, boolean isIMSOnly) {
        /// M: GEMINI+
        return placeCallGemini(context, phone, number, contactRef, isEmergencyCall, gatewayInfo,
                callGateway, -1, isIMSOnly);
    }

    public static int placeCallGemini(Context context, Phone phone,
            String number, Uri contactRef, boolean isEmergencyCall,
            RawGatewayInfo gatewayInfo, CallGatewayManager callGateway, int simId, boolean isIMSOnly) {
        ...
        try {
            ...
            connection = CallManagerWrapper.dial(phone, numberToDial, simId, isIMSOnly);
            /// @}
        } catch (CallStateException ex) {
        ...
        if ((app.getPUKEntryActivity() == null) || (state != MmiCode.State.COMPLETE)) {
            ...
            if (state != MmiCode.State.PENDING) {
                ...
                */
                ...
                ExtensionManager.getMmiCodeExt().configBeforeMmiDialogShow(mmiCode, UssdAlertActivity.USSD_DIALOG_NOTIFICATION, simId);
                ...
                ExtensionManager.getMmiCodeExt().configBeforeMmiDialogShow(mmiCode, UssdAlertActivity.USSD_DIALOG_REQUEST, simId);
                // now show the dialog!
                /// M: @{
                // Original Code:
                // newDialog.show();
    ...
    */
     ...
     */
    /* package */ public static void setAudioModeIfNeed() {
    ...
    /* package */ static boolean handleHeadsetHook(Phone phone, KeyEvent event) {
        ...
        if (phone.getState() == PhoneConstants.State.IDLE) {
            if (DBG) log("handleHeadsetHook: phone state is IDLE.");
            return false;
        }
    ...
    */
    ...
    public static boolean hangupEx(CallManager cm) {
        ...
        PhoneGlobals.getInstance().getCallModeler().requestDelayDisconnecting(call);
        /// @}
    ...
    public static boolean isSupportFeature(String feature) {
        ...
        if (feature.equals("TTY")) {
            return FeatureOption.isMtkTtySupport();
        } else if (feature.equals("DUAL_MIC")) {
            return FeatureOption.isMtkDualMicSupport();
        } else if (feature.equals("IP_DIAL")) {
            return true;
        } else if (feature.equals("3G_SWITCH")) {
            return FeatureOption.isMtkGemini3GSwitch();
            ...
            return FeatureOption.isMtkPhoneVoiceRecordingSupport();
        /// for 4G switch @{
        } else if (feature.equals("4G_SWITCH")) {
            return FeatureOption.isMtkSimSwitch();
            /// @}
        }
        return false;
    }
    ...
    */
    ...
    private static boolean forceSpeakerOn(Context context, boolean checkHeadSet) {
        ...
        if (FeatureOption.isMtkTbAppCallForceSpeakerOn()) {
            ...
            switch (cause) {
                ...
                case ICC_ERROR:
                    ...
                    resID = serviceStateExt.isImeiLocked() ? R.string.callFailed_cdma_notEmergency :
                                (FeatureOption.isEvdoDtSupport() ? R.string.callFailed_simError_cdma :
                    ...
                    if (hasMultiRingCall) {
                        if (firstRingCallIsVT) {
                            ...
                            if (fgCall == null || fgCall.getState() != Call.State.ACTIVE) {
                                fgCall = dualTalk.getSecondActiveFgCall();
                            }
                ...
                case ACTIVE_VOICE_CALL_ONLY_ONE:
                    if (hasMultiRingCall) {
                        if (firstRingCallIsVT) {
                            ...
                            // dualTalk.getActiveFgCall() is designed for get the first phone's foreground call
                            // Consider the case: the first phone is ringing and the second phone has active call
                            // see ALPS01584106
                            if (fgCall == null || fgCall.getState() != Call.State.ACTIVE) {
                                fgCall = dualTalk.getSecondActiveFgCall();
                            }
    ...
    static int getAllCallsStateMsg(CallManager callManager) {
        ...
        if (0 == activeCalls.size()) {
         ...
         if (1 == activeCalls.size() && !VTCallUtils.isVTCallActive()) {
    ...
    public static List<Call> getRingingCalls() {
    ...
    public static List<Call> getForegroundCalls() {
        List<Call> callList = new ArrayList<Call>();
        if (DualTalkUtils.isSupportDualTalk()) {
    ...
    public static List<Call> getBackgroundCalls() {
        List<Call> callList = new ArrayList<Call>();
        if (DualTalkUtils.isSupportDualTalk()) {
    ...
    private static AlertDialog sCallSelectDialog = null;

    public static void dismissCallSelectDialog() {
        if (sCallSelectDialog != null) {
            sCallSelectDialog.dismiss();
            sCallSelectDialog = null;
        }
    }
    ...
    static void showSelectCallDialog(final List<Call> holdList) {
        ...
        if (null == sCallSelectDialog || !sCallSelectDialog.isShowing()) {
            ...
            if (2 == holdList.size()) {
                ...
                builder.setSingleChoiceItems(callPickerAdapter, -1,
                        new DialogInterface.OnClickListener() {
                            public void onClick(DialogInterface dialog, int which) {
                                ...
                                Call firstBgCall = DualTalkUtils.getInstance().getFirstActiveBgCall();
                                Call secondBgCall = DualTalkUtils.getInstance().getSecondActiveBgCall();
                                // For ALPS01612401, firstBgCall and secondBgCall never is null!
                                // Should change to check the call state.
                                // original code:
                                //if (null != call && null != firstBgCall && null != secondBgCall) {
                                if (null != call && !firstBgCall.isIdle() && !secondBgCall.isIdle()) {
                ...
                sCallSelectDialog = builder.create();
                sCallSelectDialog.getWindow().setType(WindowManager.LayoutParams.TYPE_SYSTEM_ALERT);
                //Need to check whether excute this in main handler.
                sCallSelectDialog.show();
            }
        }
    }
...
/*        int support = AudioSystem.getAudioCommand(GET_SPEECH_ANC_SUPPORT);
    ...
    /// M: Change Feature  ALPS00811599 @{
    // Call not hangup when network is abnormal
    private static final String[] OPERATOR_NUMERICS = new String[] {
            "46000",
            "46001",
            "46002",
            "46007"
    ...
    public static void handlePendingCall(Object obj) {
        log("handlePendingCall!");
        Call call = (Call) obj;
        if (needReleasePendingCall(call)) {
            try {
                call.hangup();
                log("hangup pending call!");
            } catch (CallStateException ex) {
    ...
    private static boolean needReleasePendingCall(Call call) {
        boolean needRelease = false;
        String numeric = "";
        if (call == null || call.getPhone().getPhoneType() != PhoneConstants.PHONE_TYPE_GSM) {
            return needRelease;
        }

        if (call != null && call.getState() == Call.State.DISCONNECTING) {
            int slotId = call.getPhone().getMySimId();
            numeric = PhoneGlobals.getInstance().phoneMgr.getSimOperator(slotId);
            log("needReleasePendingCall(): slot id is: " + slotId + ", plmn id is: " + numeric + ".");
            for (String operatorNumeric : OPERATOR_NUMERICS) {
                if (numeric.equals(operatorNumeric)) {
                    needRelease = true;
                    break;
                }
            }
