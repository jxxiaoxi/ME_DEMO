...
public final class BluetoothNameDialogFragment extends DialogFragment implements TextWatcher {
    ...
    public Dialog onCreateDialog(Bundle savedInstanceState) {
        ...
        if (savedInstanceState != null) {
            ...
            mDeviceNameEdited = savedInstanceState.getBoolean(KEY_NAME_EDITED, true);
        }
    ...
    private View createDialogView(String deviceName) {
        ...
        mDeviceNameView.setSelection(deviceName.length());//add by ganyu   Move the cursor towards the end of text
    ...
    public void afterTextChanged(Editable s) {
        if (mDeviceNameUpdated) {
            ...
            if (mOkButton != null) {
            	//fix bug 4662:蓝牙名称可以重命名为空格
                mOkButton.setEnabled(s.toString().trim().length() != 0);
            }
        }
if (s.length() >= BLUETOOTH_NAME_MAX_LENGTH_BYTES) {
       	Toast.makeText(getActivity(), R.string.max_length_bluetooth_name, Toast.LENGTH_LONG).show();
       }
    }
