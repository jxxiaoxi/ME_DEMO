...
public class WifiApDialog extends AlertDialog implements View.OnClickListener,
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        //默认热点显示密码
        if (SystemProperties.getBoolean("ro.show.hotspot.pw", false)) {
        	if (isInitWifiAp()) {
        		((CheckBox) mView.findViewById(R.id.show_password)).setChecked(true);
        		setDisplayPw(true);
        		setInitWifiAp(false);
        	} else {
        		((CheckBox) mView.findViewById(R.id.show_password)).setChecked(isDisplayPw());
        	}
        	
        	mPassword.setInputType(
                    InputType.TYPE_CLASS_TEXT | (isDisplayPw() ?
    ...
    private void setDisplayPw(boolean display) {
    	SharedPreferences sp = PreferenceManager.getDefaultSharedPreferences(mContext);
        SharedPreferences.Editor editor = sp.edit();
        editor.putBoolean("DisplayPw", display);
        editor.commit();
    }
    
    private boolean isDisplayPw() {
        SharedPreferences sp = PreferenceManager.getDefaultSharedPreferences(mContext);
        boolean isDisplayPw = sp.getBoolean("DisplayPw", false);
        return isDisplayPw;
    }
    
    private void setInitWifiAp (boolean init) {
        SharedPreferences sp = PreferenceManager.getDefaultSharedPreferences(mContext);
        SharedPreferences.Editor editor = sp.edit();
        editor.putBoolean("InitWifiAp", init);
        editor.commit();
    }

    private boolean isInitWifiAp () {
        SharedPreferences sp = PreferenceManager.getDefaultSharedPreferences(mContext);
        boolean isFristInit = sp.getBoolean("InitWifiAp", true);
        return isFristInit;
    }
    
    private void validate() {//fix bug 4853:WIFI便捷式热点可以重命名为空格
    ...
    public void onClick(View view) {
        if (view.getId() == R.id.show_password) {
            ...
            setDisplayPw(((CheckBox) view).isChecked());
