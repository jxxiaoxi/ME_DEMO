...
public class DeviceInfoSettings extends RestrictedSettingsFragment {
    ...
    public void onCreate(Bundle icicle) {
        ...
        if (FeatureOption.CUSTOM_ALCATEL_4031D_MM) {
        	setStringSummary(KEY_DEVICE_MODEL, "T400M");
        } else if (FeatureOption.CUSTOM_HAIER_ES_US) {
        	setStringSummary(KEY_DEVICE_MODEL, "W717(G10)");
        } else {
        	setStringSummary(KEY_DEVICE_MODEL, Build.MODEL);
        }
        
        //brand
        if (SystemProperties.getBoolean("ro.display.brand", false)) {
        	if (FeatureOption.CUSTOM_HAIER_ZUUM) {
        		setStringSummary(KEY_DEVICE_BRAND, "ZUUM");
        	}
        } else {
	...
	if (FeatureOption.CUSTOM_HAIER_ZUUM) {
			 ...
			 setStringSummary(KEY_BUILD_NUMBER, Build.DISPLAY.replace("LIV_I","LIV I"));		
        ...
        setStringSummary("hardware_version", Build.HARDWARE_VERSION);
	if (FeatureOption.CUSTOM_ALCATEL) 
	{	
		//getPreferenceScreen().removePreference(findPreference("hardware_version"));
	}
	...
	if(findPreference(KEY_BASEBAND_VERSION_2) != null)
        ...
        removePreference(KEY_SYSTEM_UPDATE_SETTINGS);//add huangzs remove KEY_SYSTEM_UPDATE_SETTINGS
