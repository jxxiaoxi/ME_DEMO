...
public class AudioProfileSettings extends SettingsPreferenceFragment implements
    ...
    private static String mRenameDialogtext;
    ...
    public void onCreateOptionsMenu(Menu menu, MenuInflater inflater) {
        // modified by sijingjing
        if (FeatureOption.HY_WHITE_THEME_SETTINGS) {
            ...
            menu.add(0, MENUID_RESET, 0, R.string.audio_profile_reset).setIcon(
                    R.drawable.ic_menu_reset_light).setShowAsAction(
                            MenuItem.SHOW_AS_ACTION_IF_ROOM
                                    | MenuItem.SHOW_AS_ACTION_WITH_TEXT);
        } else {
            menu.add(0, MENUID_ADD, 0, R.string.audio_profile_add)
        ...
        if (id == DIALOG_NAME) {
            ...
            if (mEditText != null) {
                ...
                mEditText.setText(mMenuId == MENUID_ADD ? ""
                        : mRenameDialogtext);
                //fix bug 4309:设置情景模式，重命名新建的情景模式，光标在字符的前面 
                if (mMenuId == MENUID_RENAME) {
                	if (mRenameDialogtext != null) { 
    ...
    public void onClick(DialogInterface dialogInterface, int button) {
        ...
        case DIALOG_NAME:
            String title = mEditText == null ? "" : String.valueOf(mEditText
                    .getText());
            if (title.toString().trim().length() == 0) {//Fix bug 7140:重命名新建情景模式可命名为空格
        ...
        case DIALOG_RESET:
            ...
            /*add by ganyu begin reset BesAudEnh*/
            if (FeatureOption.MTK_AUDENH_SUPPORT) {
            	mMusicPlusPrf.setChecked(false);
