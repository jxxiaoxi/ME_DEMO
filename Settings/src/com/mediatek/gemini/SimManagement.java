...
public class SimManagement extends SettingsPreferenceFragment implements
	...
	List<Boolean> itemStatusListgprs = new ArrayList<Boolean>();	//add huangzs
	    ...
	    if(FeatureOption.CUSTOM_ALCATEL_4031D_PH)//add huangzs
	    {
                if(checkSim2Lock(siminfo.mSimSlotId))
		{
	    	  itemStatusListgprs.add(false);
		}
		else
		{
		  itemStatusListgprs.add(true);
		}
	    }
	...
	if(FeatureOption.CUSTOM_ALCATEL_4031D_PH)
	{
		mGprsSimSetting.setEntriesData(mSimInfoList, simIndicatorList, normalListGprs, itemStatusListgprs);
	}
        else
        {
   ...
   private boolean checkSim2Lock(int slotId) {
        if(slotId==PhoneConstants.GEMINI_SIM_2)
	{
	 	try {
		    String imsiId = mTelephonyEx.getSubscriberId(PhoneConstants.GEMINI_SIM_2);
                    if(imsiId!=null)
		    {
			if(!imsiId.startsWith("51502"))
			return true;
		    }
		} catch (android.os.RemoteException ex) {
		    Xlog.d(TAG, "RemoteException when checkSim");
		}
	}
        return false;
    }
    ...
    public Dialog onCreateDialog(int id) {
        ...
        case DIALOG_GPRS_SWITCH_CONFIRM:
            ...
            builder.setIcon(R.drawable.ic_dialog_alert); // modified by sijingjing 
