        ...
        public void onClick(DialogInterface dialogInterface, int button) {
            if (button == BUTTON_SUBMIT) {
                 Xlog.d(TAG, "onClick, save configuration");
                  mWpsListener = new WifiManager.WpsListener() {
                    ...
                    public void onFailure(int reason) {
                        Xlog.d(TAG, "onFailure, the reason is :" + reason);
                ...
                if (mIsOnlyVisibilityWpsPin) {
                    if (mAccessPoint == null || mAccessPoint.networkId != INVALID_NETWORK_ID) {
                        Xlog.d(TAG, "startWpsEr, config = " + config.toString());
                        mWifiManager.startWpsEr(config, mWpsListener);
                    } else {
                    ...
                    if (!nfcPasswordCheckBox.isChecked() && (mPinCode.getText() == null || "".equals(mPinCode.getText()))) {
                        ...
                        config.key = mKey.getText().toString();
                         ...
                         Xlog.d(TAG, "startWpsReg, config = " + config.toString());
                        mWifiManager.startWpsReg(config, mWpsListener);
                    }
                }
            }
