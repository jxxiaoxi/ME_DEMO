...
public class DisplaySettingsExt implements OnPreferenceClickListener {
    ...
    private void initPreference(PreferenceScreen screen) {
        ...
        // add for clearMotion
        mClearMotion = (CheckBoxPreference) createPreference(TYPE_CHECKBOX,
                R.string.clear_motion_title, KEY_CLEAR_MOTION, mDisplayPerCategory);
        ...
        mLockScreenNotifications = (CheckBoxPreference) createPreference(TYPE_CHECKBOX,
                R.string.lock_screen_notifications_title, KEY_LOCK_SCREEN_NOTIFICATIONS,
                mDisplayPerCategory);
        mLockScreenNotifications.setSummary(R.string.lock_screen_notifications_summary);

        if (FeatureOption.MTK_THEMEMANAGER_APP) {
            mColorPref = createPreference(TYPE_PREFERENCE, R.string.mtk_thememanager_title,
                    KEY_COLOR, mDisplayPerCategory);
        ...
        // remove Daydream when MTK_LCA_RAM_OPTIMIZE is true
    ...
    public void onResume() {
        ...
        // add for clearMotion
        mContext.registerReceiver(mUpdateClearMotionStatusReceiver, new IntentFilter(
    ...
    public boolean onPreferenceClick(Preference preference) {
        if (preference == mLockScreenStylePref) {
            ...
            Settings.System.putInt(mContext.getContentResolver(), INCOMING_INDICATOR_ON_LOCKSCREEN,
                    value ? 1 : 0);
    ...
    /**
     * Get key guard layers from system, a key guard layer should implement
     * IKeyguardLayer interface. Plugin app should make sure the data is valid.
     */
        ...
        try {
            ...
            if (pluginCount != 0) {
                ...
                for (int i = 0; i < pluginCount; i++) {
                    ...
                    Xlog.d(TAG, "getKeyguardLayers: i = " + i + ",keyguardLayer = " + keyguardLayer
                            + ",info = " + info);
                    if (info != null) {
                        pluginLayers = true;
                        return pluginLayers;
    ...
    /**
     * add for clearMotion clearMotionStyle clearMotion^TM
     */
    ...
    /**
     * add for clearMotion clearMotion and WFD/HDMI can't co-work
     */
    private void updateClearMotionStatus() {
        if (mClearMotion != null) {
            Xlog.d(TAG, "updateClearMotionStatus");
            mClearMotion
                    .setChecked(SystemProperties.get(KEY_DISPLAY_CLEAR_MOTION, "0").equals("1"));
            mClearMotion.setEnabled(SystemProperties.get(KEY_DISPLAY_CLEAR_MOTION_DIMMED, "0")
                    .equals("0"));
        }
    }
