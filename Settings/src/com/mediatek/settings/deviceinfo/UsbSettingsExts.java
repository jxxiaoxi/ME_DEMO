...
public class UsbSettingsExts {
	...
	public PreferenceScreen addUsbSettingsItem(UsbSettings usbSettings) {
		...
		Xlog.d(TAG, TAG + ".addUsbSettingsItem() isAbleToShare() = " + isAbleToShare());
		
		if (umsExist && isAbleToShare()) {
		...
		boolean bicrExist;
		if (FeatureOption.CUSTOM_HAIER) 
		bicrExist = false;
		else
		bicrExist = bicrConfig.equals(FUNCTION_SUPPORT);
		if (bicrExist) {
	...
	public boolean isAbleToShare() {
        ...
        if (volumes != null) {
            Xlog.d(TAG, "isAbleToShare - length:" + volumes.length);
            for (int i = 0; i < volumes.length; i++) {
            	Xlog.d(TAG, "isAbleToShare - allowMassStorage:" + volumes[i].allowMassStorage() + "isEmulated:" + volumes[i].isEmulated());
                if (volumes[i].allowMassStorage() && !volumes[i].isEmulated()) {
                    String path = volumes[i].getPath();
                    String st = mStorageManager.getVolumeState(path);
                    if (st != null) {
                    	Xlog.d(TAG, String.format("isAbleToShare - %s @ %s", path, st));
        ...
        Xlog.d(TAG, "isAbleToShare - allowedShareNum:" + allowedShareNum);
